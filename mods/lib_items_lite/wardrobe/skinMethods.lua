...
local playerMesh = "3d_armor_character_sf.b3d";
do  -- autodetect version of player mesh used by default
   if gal and gal.player.api.registered_models then
      local haveCharName = false;  -- 'character.*' has priority
      local name = nil;
      local nNames = 0;

      for k in pairs(gal.player.api.registered_models) do
...
local SKIN_CHANGE_METHODS =
   {
      ...
      ["3d_armor"] =
      {
         ...
         changeSkin = function(playerName, skin)
            ...
            gal.player.armor.textures[playerName].skin = skin;
         end,

         updateSkin = function(player)
            gal.player.armor:update_player_visuals(player);
         end,
      },
   };
