...
--minetest.override_item("gal:liquid_water_source", {
--	liquid_renewable = false,
--})

--minetest.override_item("gal:liquid_water_flowing", {
--	liquid_renewable = false,
--})
...
local function barrel_drain(pos,_,player)
	...
	local container = player:get_wielded_item():get_name()
	...
	if not (string.find(container, "bucket") or string.find(container, "pot_clay")) and (string.find(container, "emtpy")) then
		return
	end
	...
	if water >= 1.0 then --Code copied from buckets
		...
		local giving_back = string.gsub(container, "empty", "water")
		if item_count > 1 then
			...
			giving_back = container .. " "..tostring(item_count-1)
		end
...
minetest.register_node("rainbarrel:well", {
	...
	paramtype = "light",
	groups = {cracky=3},
	tiles = {minetest.registered_nodes["gal:liquid_water_source"].tiles[1],"default_wood.png","default_stone.png","default_cobble.png"},
