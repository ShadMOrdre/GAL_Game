...
local function get_stand_object(pos)
	...
	for _, obj in pairs(objects) do
		...
		if ent then
			if ent.name == "lib_armor:armor_stand_entity" then
				-- Remove duplicates
				if object then
					obj:remove()
				else
...
local function update_entity(pos)
	...
	if object then
		if not string.find(node.name, "lib_armor:") then
			object:remove()
			return
		end
	else
		object = minetest.add_entity(pos, "lib_armor:armor_stand_entity")
	end
	if object then
...
local function add_hidden_node(pos, player)
	...
	if node.name == "air" and not minetest.is_protected(pos, name) then
		minetest.set_node(p, {name="lib_armor:top"})
	end
end
...
local function remove_hidden_node(pos)
	local p = {x=pos.x, y=pos.y + 1, z=pos.z}
	local node = minetest.get_node(p)
	if node.name == "lib_armor:top" then
		minetest.remove_node(p)
	end
end

minetest.register_node("lib_armor:top", {
...
minetest.register_node("lib_armor:armor_stand", {
	...
	sounds = lib_armor.node_sound_wood_defaults(),
	...
	after_place_node = function(pos, placer)
		minetest.add_entity(pos, "lib_armor:armor_stand_entity")
	...
	on_blast = function(pos)
		drop_armor(pos)
		gal.player.armor.drop_armor(pos, "lib_armor:armor_stand")
		minetest.remove_node(pos)
	end,
})

minetest.register_node("lib_armor:armor_stand_locked", {
...
minetest.register_entity("lib_armor:armor_stand_entity", {
	physical = true,
	visual = "mesh",
	...
	on_blast = function(self, damage)
		...
		if node.name == "lib_armor:armor_stand" then
			drop_armor(self.pos)
			self.object:remove()
		end
...
minetest.register_abm({
	nodenames = {"lib_armor:armor_stand_locked", "lib_armor:armor_stand"},
	interval = 15,
	chance = 1,
...
minetest.register_craft({
	output = "lib_armor:armor_stand_locked",
	recipe = {
		{"lib_armor:armor_stand", "default:steel_ingot"},
	}
})
