...
function lib_forge.crafting.add_craft(output, recipe)
	local output_name = string.gsub(output, ":", "_", 1)
	lib_forge.crafts[output_name] = recipe
end
...
local function check_craft(pos, list)
	...
	local wear = 0
	local x = {}
	for i,v in pairs(lib_forge.crafts) do
...
local function dfurnace_node_timer(pos, elapsed)
	...
	if fuel_time <= fuel_totaltime and fuel_totaltime ~= 0 then
		swap_node(pos, "lib_forge:dual_furnace_active")
		result = true
		...
		swap_node(pos, "lib_forge:dual_furnace_inactive")
...
for _, nn in pairs({"active", "inactive"}) do
	...
	minetest.register_node("lib_forge:dual_furnace_" .. nn, {
		description = "Dual Furnace",
		tiles = {
			"lib_forge_furnace_top.png",
			"lib_forge_furnace_bottom.png",
			"lib_forge_furnace_alloy_side.png",
			"lib_forge_furnace_alloy_side.png",
			"lib_forge_furnace_alloy_side.png",
			"lib_forge_furnace_alloy_front_" .. nn .. ".png"
		},
		paramtype2 = "facedir",
		groups = groups,
		is_ground_content = false,
		paramtype = "light",
		drop = "lib_forge:dual_furnace_inactive",
...
minetest.register_craft({
	output = "lib_forge:dual_furnace_inactive",
	recipe = {
		{"", "default:steel_ingot", ""},
...
lib_forge.crafting.add_craft("default:bronze_ingot", {
	recipe = {"default:tin_ingot", "default:copper_ingot"},
	type = "dualfurn"
})

lib_forge.crafting.add_craft("default:bronze_ingot", {
	recipe = {"default:tin_lump", "default:copper_lump"},
	type = "dualfurn"
})
