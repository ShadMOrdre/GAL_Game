...
--local S = lib_forge.gettext
local S = lib_forge.intllib



-- the hammer for the anvil
minetest.register_tool("lib_forge:anvil_hammer", {
	--description = S("Steel blacksmithing hammer"),
	original_description = S("Steel blacksmithing hammer"),
	description = toolranks.create_description("Steel blacksmithing hammer", 0, 1),
...
minetest.register_entity("lib_forge:anvil_item",{
	hp_max = 1,
	visual="wielditem",
...
local remove_item = function(pos, node)
	...
	if objs then
		for _, obj in ipairs(objs) do
			if obj and obj:get_luaentity() and obj:get_luaentity().name == "lib_forge:anvil_item" then
				obj:remove()
			end
		end
	end
...
local update_item = function(pos, node)
	...
	if not inv:is_empty("input") then
		...
		local e = minetest.add_entity(pos,"lib_forge:anvil_item")
		local yaw = math.pi*2 - node.param2 * math.pi/2
		e:setyaw(yaw)
	end
end

local metal_sounds = lib_forge.node_sound_metal_defaults()

minetest.register_node("lib_forge:anvil", {
	drawtype = "nodebox",
	description = S("Anvil"),
	...
	on_punch = function(pos, node, puncher)
		...
		if wielded:get_name() ~= 'lib_forge:anvil_hammer' then
			return
		end
...
minetest.register_lbm({
	name = "lib_forge:anvil_item_restoration",
	nodenames = { "lib_forge:anvil" },
...
minetest.register_lbm({
	name = "lib_forge:hammer_ejection",
	nodenames = "lib_forge:anvil",
