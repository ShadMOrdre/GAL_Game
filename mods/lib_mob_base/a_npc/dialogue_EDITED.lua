...
a_npc.conversation.POSITIVE_GIFT_ANSWER_PREFIX = "Yes, give "
a_npc.conversation.NEGATIVE_ANSWER_LABEL = "Nevermind"

a_npc.conversation.MIN_DIALOGUES = 2
a_npc.conversation.MAX_DIALOGUES = 4

a_npc.conversation.dialogue_type = {
  married = 1,
  casual_trade = 2,
  dedicated_trade = 3,
  custom_trade = 4
...
a_npc.conversation.dialogue_results = {
	options_dialogue = {},
	yes_no_dialogue = {}
}
...
function a_npc.conversation.show_options_dialogue(self, dialogue, dismiss_option_label, player_name) 
  local responses = a_npc.conversation.responses
	...
	a_npc.conversation.dialogue_results.options_dialogue[player_name] = {
		npc = self,
		is_married_dialogue = (a_npc.conversation.dialogue_type == a_npc.conversation.dialogue_type.married),
		is_casual_trade_dialogue = (a_npc.conversation.dialogue_type == a_npc.conversation.dialogue_type.casual_trade),
    is_dedicated_trade_dialogue = (a_npc.conversation.dialogue_type == a_npc.conversation.dialogue_type.dedicated_trade),
    is_custom_trade_dialogue = (a_npc.conversation.dialogue_type == a_npc.conversation.dialogue_type.custom_trade),
		casual_trade_type = a_npc.conversation.casual_trade_type,
		options = responses
	}
...
function a_npc.conversation.show_yes_no_dialogue(self,
                                           prompt, 
  ...
  a_npc.lock_actions(self)
	...
	a_npc.conversation.dialogue_results.yes_no_dialogue[player_name] = {
    npc = self,
...
local function set_response_ids_recursively(dialogue, depth, dialogue_id)
  ...
  if a_npc.conversation.responses == nil 
    and (a_npc.conversation.action_type == "dialogue" and a_npc.conversation.action.responses == nil) then
    return
  elseif a_npc.conversation.responses ~= nil then
    -- Assign a response ID to each response
    local response_id_prefix = tostring(depth)..":"
    for key,value in ipairs(a_npc.conversation.responses) do
...
function a_npc.conversation.select_random_dialogues_for_npc(sex, phase, favorite_items, disliked_items)
	local result = {
		normal = {},
		hints = {}
	}

	local dialogues = a_npc.data.DIALOGUES.female
	if sex == a_npc.MALE then
		dialogues = a_npc.data.DIALOGUES.male
	end
	dialogues = dialogues[phase]

	-- Determine how many dialogue lines the NPC will have
	local number_of_dialogues = math.random(a_npc.conversation.MIN_DIALOGUES, a_npc.conversation.MAX_DIALOGUES)
	...
	for i = 1, 2 do
		result.hints[i] = {}
		result.hints[i].text = 
			a_npc.relationships.get_hint_for_favorite_item(favorite_items["fav"..tostring(i)], sex, phase)
	end
	...
	for i = 3, 4 do
		result.hints[i] = {}
		result.hints[i].text = 
			a_npc.relationships.get_hint_for_disliked_item(disliked_items["dis"..tostring(i-2)], sex)
	end
...
-- NPC have.
function a_npc.conversation.create_custom_trade_options(self, player)
  -- Create the action for each option
  local actions = {}
  for i = 1, #self.trader_data.custom_trades do
    table.insert(actions, function() a_npc.trade.show_custom_trade_offer(self, player, self.trader_data.custom_trades[i]) end)   
  end
  -- Default text to be shown for dialogue prompt
  local text = a_npc.trade.CUSTOM_TRADES_PROMPT_TEXT
  ...
  local dialogue = a_npc.conversation.create_option_dialogue(text, options, actions)
  a_npc.conversation.dialogue_type = a_npc.conversation.dialogue_type.custom_trade
...
-- and process it. 
function a_npc.conversation.start_dialogue(self, player, show_married_dialogue)
	-- Choose a dialogue randomly
	local dialogue = {}

	-- Construct dialogue for marriage
	if a_npc.relationships.get_relationship_phase(self, player:get_player_name()) == "phase6"
		and show_married_dialogue == true then
		dialogue = a_npc.relationships.MARRIED_NPC_DIALOGUE
		a_npc.conversation.process_dialogue(self, dialogue, player:get_player_name())
		return
	end 

  -- Show options dialogue for dedicated trader
  if self.trader_data.trader_status == a_npc.trade.TRADER then
    dialogue = a_npc.trade.DEDICATED_TRADER_PROMPT
    a_npc.conversation.process_dialogue(self, dialogue, player:get_player_name())
    return
  end
    ...
    if self.trader_data.trader_status == a_npc.trade.NONE then
      -- Show custom trade options if available
      if table.getn(self.trader_data.custom_trades) > 0 then
        -- Show custom trade options
        dialogue = a_npc.conversation.create_custom_trade_options(self, player)
      end
    elseif self.trader_data.trader_status == a_npc.trade.CASUAL then
      ...
      if table.getn(self.trader_data.custom_trades) > 0 then
        max_trade_chance = 3
  		...
  		if trade_chance == 1 then
  			...
  			dialogue = a_npc.trade.CASUAL_TRADE_BUY_DIALOGUE
  		elseif trade_chance == 2 then
  			-- Show casual sell dialogue
  			dialogue = a_npc.trade.CASUAL_TRADE_SELL_DIALOGUE
	...
	local dialogue_result = a_npc.conversation.process_dialogue(self, dialogue, player:get_player_name())
  if dialogue_result == false then
    -- Try to find another dialogue line
    a_npc.conversation.start_dialogue(self, player, show_married_dialogue)
  end
end

-- This function processes a dialogue object and performs
-- actions depending on what is defined in the object 
function a_npc.conversation.process_dialogue(self, dialogue, player_name)

  -- Freeze NPC actions
  a_npc.lock_actions(self)

  -- Check if this dialogue has a flag definition
  if a_npc.conversation.flag then
    -- Check if the NPC has this flag
    local flag_value = a_npc.get_flag(self, a_npc.conversation.flag.name)
    if flag_value ~= nil then
      -- Check if value of the flag is equal to the expected value
      if flag_value ~= a_npc.conversation.flag.value then
        -- Do not process this dialogue
        return false
      end
    else
      
      if (type(a_npc.conversation.flag.value) == "boolean" and a_npc.conversation.flag.value ~= false)
        or (type(a_npc.conversation.flag.value) == "number" and a_npc.conversation.flag.value > 0) then
        -- Do not process this dialogue
        return false
      end
    end
	...
	-- Send dialogue line
	if a_npc.conversation.text then
		minetest.chat_send_player(player_name, self.nametag..": "..a_npc.conversation.text)
	end

  -- Check if dialogue has responses. If it doesn't, unlock the actions
  -- queue and reset actions timer.'
  if not a_npc.conversation.responses then
    a_npc.unlock_actions(self)
  end

	-- Check if there are responses, then show multi-option dialogue if there are
	if a_npc.conversation.responses then
		a_npc.conversation.show_options_dialogue(
			self,
			dialogue,
...
function a_npc.conversation.create_option_dialogue(prompt, options, actions)
  local result = {}
  result.text = prompt
  result.responses = {}
  for i = 1, #options do
...
function a_npc.conversation.rotate_npc_to_player(self)
...
local function get_response_object_by_id_recursive(dialogue, current_depth, response_id)
  if a_npc.conversation.responses == nil 
    and (a_npc.conversation.action_type == "dialogue" and dialoge.action.responses == nil) then
    return nil
  elseif a_npc.conversation.responses ~= nil then
...
minetest.register_on_player_receive_fields(function (player, formname, fields)
	...
	if formname == "advanced_npc:yes_no" then
		...
		if fields then
			local player_response = a_npc.conversation.dialogue_results.yes_no_dialogue[player_name]

      -- Unlock queue, reset action timer and unfreeze a_npc.
	...
	if formname == "advanced_npc:options" then
		...
		if fields then
			-- Get player response
			local player_response = a_npc.conversation.dialogue_results.options_dialogue[player_name]
        ...
        a_npc.unlock_actions(player_response.npc)
			...
			for i = 1, #player_response.options do
				...
				if fields[button_label] then
					if player_response.options[i].action_type == "dialogue" then
						...
						a_npc.conversation.process_dialogue(player_response.npc, 
					...
					elseif player_response.options[i].action_type == "function" then
						...
						local phase = 
							a_npc.relationships.get_relationship_phase(player_response.npc, player_name)
						...
						elseif player_response.is_casual_trade_dialogue == true then
							...
							if player_response.casual_trade_type == a_npc.trade.OFFER_BUY then
								-- Get functions from casual buy dialogue
								a_npc.trade.CASUAL_TRADE_BUY_DIALOGUE
									.responses[player_response.options[i].response_id]
									.action(player_response.npc, player)
							elseif player_response.casual_trade_type == a_npc.trade.OFFER_SELL == true then
            ...
            elseif player_response.is_custom_trade_dialogue == true then
              ...
              player_response.options[i].action(player_response.npc, player)
							...
							local dialogues = a_npc.data.DIALOGUES[player_response.a_npc.sex][phase]
