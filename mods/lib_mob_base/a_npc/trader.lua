...
a_npc.human = {
	items = {
		...
		{"gal:brick 10", "gal:metal_gold_ingot 4", 17},
		...
		{"gal:stne_desert 10", "gal:metal_gold_ingot 8", 27},
...
mobs:register_mob("a_npc:a_trader", {
	type = "npc",
	passive = false,
	damage = 3,
	...
	on_rightclick = function(self, clicker)
		a_npc_trader(self, clicker, entity, a_npc.human)
	end,
})
...
function a_npc.allow_move(inv, from_list, from_index, to_list, to_index, count, player)
...
function a_npc.allow_put(inv, listname, index, stack, player)
...
function a_npc.allow_take(inv, listname, index, stack, player)
...
function a_npc.on_put(inv, listname, index, stack)

	if listname == "payment" then
		a_npc.update_takeaway(inv)
	end
end

function a_npc.on_take(inv, listname, count, index, stack, player)
	...
	if listname == "payment" then
		...
		if a_npc.check_pay(inv, false) then
...
function a_npc.update_takeaway(inv)
...
function a_npc.check_pay(inv, paynow)
...
a_npc.trader_inventories = {}

function a_npc.add_goods(entity, race)
	...
	for i = 1, 15 do
		...
		if math.random(0, 100) > race.items[i][3] then
			a_npc.trader_inventory.set_stack(a_npc.trader_inventory,
				"goods", i, race.items[i][1])
		end
	end
end

function a_npc_trader(self, clicker, entity, race)
	...
	local move_put_take = {
		...
		allow_move = a_npc.allow_move,
		allow_put = a_npc.allow_put,
		allow_take = a_npc.allow_take,
		...
		on_put = a_npc.on_put,
		on_take = a_npc.on_take
	}

	if is_inventory == nil then

		a_npc.trader_inventory = minetest.create_detached_inventory(unique_entity_id, move_put_take)
		a_npc.trader_inventory.set_size(a_npc.trader_inventory,"goods", 15)
		a_npc.trader_inventory.set_size(a_npc.trader_inventory,"takeaway", 1)
		a_npc.trader_inventory.set_size(a_npc.trader_inventory,"selection", 1)
		a_npc.trader_inventory.set_size(a_npc.trader_inventory,"price", 1)
		a_npc.trader_inventory.set_size(a_npc.trader_inventory,"payment", 1)
		a_npc.add_goods(entity, race)
	end
...
mobs:register_egg("a_npc:a_trader", S("A Trader"), "default_sandstone.png", 1)
