...
function a_npc.getVillagerName(gender, region)
	local name
	if region then
...
function a_npc.getVillagerAppearance(building_type, region, gender, age)
...
a_npc.random_appearence = function()
...
--
	...
	if (t_region == "native") or (t_region == "desert") then
		villager_name = a_npc.getVillagerName(t_gender, t_region)
	else
		if t_age == "young" then
			villager_name = a_npc.getVillagerName(t_gender, t_age)
		else
			villager_name = a_npc.getVillagerName(t_gender)
		end
	end
--

	--local newTexture, newSize, newCollisionBox = a_npc.getVillagerAppearance(p_build[math.random(1,3)], p_region[math.random(1,5)], p_gender[math.random(1,2)], p_age[math.random(1,3)])
	local newTexture, newSize, newCollisionBox = a_npc.getVillagerAppearance(p_build[math.random(1,3)], t_region, t_gender, t_age)
