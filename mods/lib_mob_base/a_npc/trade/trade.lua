...
a_npc.trade.CASUAL = "casual"
a_npc.trade.TRADER = "trader"
a_npc.trade.NONE = "none"

a_npc.trade.OFFER_BUY = "buy"
a_npc.trade.OFFER_SELL = "sell"

-- This variable establishes how much items a dedicated
-- trader will buy until retiring the offer
a_npc.trade.DEDICATED_MAX_BUY_AMOUNT = 5
...
a_npc.trade.CUSTOM_TRADES_PROMPT_TEXT = "Hi there, how can I help you today?"

-- Casual trader NPC dialogues definition
-- Casual buyer
a_npc.trade.CASUAL_TRADE_BUY_DIALOGUE = {
  text = "I'm looking to buy some items, are  you interested?",
  casual_trade_type = a_npc.trade.OFFER_BUY,
  dialogue_type = a_npc.conversation.dialogue_type.casual_trade,
  responses = {
    [1] = {
      text = "Sell",
      ...
      action = function(self, player)
        a_npc.trade.show_trade_offer_formspec(self, player, a_npc.trade.OFFER_BUY)
      end
    }
  }
}
...
-- Casual seller
a_npc.trade.CASUAL_TRADE_SELL_DIALOGUE = {
  text = "I have some items to sell, are you interested?",
  dialogue_type = a_npc.conversation.dialogue_type.casual_trade,
  casual_trade_type = a_npc.trade.OFFER_SELL,
  responses = {
    [1] = {
      text = "Buy",
      action_type = "function",
      response_id = 1,
      action = function(self, player)
        a_npc.trade.show_trade_offer_formspec(self, player, a_npc.trade.OFFER_SELL)
      end
    }
  }
}
...
a_npc.trade.DEDICATED_TRADER_PROMPT = {
  text = "Hello there, would you like to trade?",
  dialogue_type = a_npc.conversation.dialogue_type.dedicated_trade,
  responses = {
    [1] = {
      text = "Buy",
      action_type = "function",
      response_id = 1,
      action = function(self, player)
        a_npc.trade.show_dedicated_trade_formspec(self, player, a_npc.trade.OFFER_SELL)
      end
    },
    [2] = {
      text = "Sell",
      ...
      response_id = 2,
      action = function(self, player)
        a_npc.trade.show_dedicated_trade_formspec(self, player, a_npc.trade.OFFER_BUY)
      end
    },
    [3] = {
      text = "Other",
      ...
      response_id = 3,
      action = function(self, player)
        local dialogue = a_npc.conversation.create_custom_trade_options(self, player)
        a_npc.conversation.process_dialogue(self, dialogue, player:get_player_name())
      end
    }
  }
}

function a_npc.trade.show_trade_offer_formspec(self, player, offer_type)
  ...
  if offer_type == a_npc.trade.OFFER_SELL then
  ...
  local formspec = "size[8,4]"..
                ...
                "button_exit[4.1,3.3;2.9,0.5;no_option;"..a_npc.conversation.NEGATIVE_ANSWER_LABEL.."]" 

  -- Create entry into results table
  a_npc.trade.results.single_trade_offer[player:get_player_name()] = {
...
function a_npc.trade.show_dedicated_trade_formspec(self, player, offers_type)

  -- Choose the correct offers
  local offers = self.trader_data.buy_offers
  local menu_offer_type = "sell"
  if offers_type == a_npc.trade.OFFER_SELL then
  ...
  for i = 1, #offers do
    local price_item_name = minetest.registered_items[a_npc.get_item_name(offers[i].price[1])].description
    local count_label = ""
    if a_npc.get_item_count(offers[i].item) > 1 then
      count_label = "label["..(current_x + 1.35)..","..(current_y + 1)..";"..a_npc.get_item_count(offers[i].item).."]"
    end
    formspec = formspec.. 
      "box["..current_x..","..current_y..";2,2.3;#212121]"..
      "item_image["..(current_x + 0.45)..","..(current_y + 0.15)..";1.3,1.3;"..a_npc.get_item_name(offers[i].item).."]"..
      count_label..
  ...
  formspec = formspec .. "button_exit[2.5,7.9;3.9,0.5;exit;Nevermind]"
   ...
   -- Create entry into results table
  a_npc.trade.results.trade_offers[player:get_player_name()] = {
...
function a_npc.trade.show_custom_trade_offer(self, player, offer)
  local for_string = "for"
  ...
  -- Create entry into results table
  a_npc.trade.results.custom_trade_offer[player:get_player_name()] = {
    trade_offer = offer,
    npc = self
  }
  -- Show formspec to player
...
function a_npc.trade.get_random_trade_status()
	local chance = math.random(1,10)

	if chance < 3 then
		-- Non-trader
    return a_npc.trade.NONE
	elseif 3 <= chance and chance <= 7 then
    -- Casual trader
    return a_npc.trade.CASUAL
	elseif chance > 7 then
    -- Trader by profession
    return a_npc.trade.TRADER
	end
end

-- This function generates and stores on the NPC data trade
-- offers depending on the trader status. 
function a_npc.trade.generate_trade_offers_by_status(self)
  -- Get trader status
  local status = self.trader_data.trader_status
  -- Check what is the trader status
  if status == a_npc.trade.NONE then
    -- For none, clear all offers
    self.trader_data.buy_offers = {}
    self.trader_data.sell_offers = {}
  elseif status == a_npc.trade.CASUAL then
    -- For casual, generate one buy and one sell offer
    self.trader_data.buy_offers = {
      [1] = a_npc.trade.get_casual_trade_offer(self, a_npc.trade.OFFER_BUY)
    }
    self.trader_data.sell_offers = {
      [1] = a_npc.trade.get_casual_trade_offer(self, a_npc.trade.OFFER_SELL)
    }
  elseif status == a_npc.trade.TRADER then
    -- Clear current offers
    self.trader_data.buy_offers = {}
    self.trader_data.sell_offers = {}
    -- Get trade offers for a dedicated trader
    local offers = a_npc.trade.get_dedicated_trade_offers(self)
    -- Store buy offers
...
function a_npc.trade.get_currencies_in_inventory(self)
  local result = {}
  local tier3 = a_npc.inventory_contains(self, a_npc.trade.prices.currency.tier3.string)
  local tier2 = a_npc.inventory_contains(self, a_npc.trade.prices.currency.tier2.string)
  local tier1 = a_npc.inventory_contains(self, a_npc.trade.prices.currency.tier1.string)
  if tier3 ~= nil then
    table.insert(result, {name = a_npc.get_item_name(tier3.item_string), 
                          count = a_npc.get_item_count(tier3.item_string)} )
  end
  if tier2 ~= nil then
    table.insert(result, {name = a_npc.get_item_name(tier2.item_string), 
                          count = a_npc.get_item_count(tier2.item_string)} )
  end
  if tier1 ~= nil then
    table.insert(result, {name = a_npc.get_item_name(tier1.item_string), 
                          count = a_npc.get_item_count(tier1.item_string)} )
  end
...
function a_npc.trade.get_casual_trade_offer(self, offer_type)
  local result = {}
  -- Check offer type
  if offer_type == a_npc.trade.OFFER_BUY then
    -- Create buy offer based on what the NPC can actually buy
    local currencies = a_npc.trade.get_currencies_in_inventory(self)
    -- Choose a random currency
    local chosen_tier = currencies[math.random(#currencies)]
    -- Get items for this currency
    local buyable_items =
      a_npc.trade.prices.get_items_for_currency_count(chosen_tier.name, chosen_tier.count, 0.5)
    ...
    result = a_npc.trade.create_offer(a_npc.trade.OFFER_BUY, item, buyable_items[item].price, buyable_items[item].min_buyable_item_price, amount_to_buy)
  else
    ...
    for i = 1, #self.inventory do
      if self.inventory[i] ~= "" then
        if a_npc.trade.prices.is_item_currency(self.inventory[i]) == false then
    ...
    local count = math.random(a_npc.get_item_count(item))
    -- Create trade offer
    result = a_npc.trade.create_offer(a_npc.trade.OFFER_SELL, a_npc.get_item_name(item), nil, nil, count)
  end
...
-- inventories,
function a_npc.trade.get_dedicated_trade_offers(self)
  local offers = { 
    sell = {},
    buy = {}
  }
  ...
  for item_name, trade_info in pairs(trade_list) do
    ...
    local item = a_npc.inventory_contains(self, item_name)
    if item ~= nil and trade_info.last_offer_type ~= a_npc.trade.OFFER_BUY then
        ...
        -- sell is made.
        table.insert(offers.sell, a_npc.trade.create_offer(
          a_npc.trade.OFFER_SELL, 
          item_name, 
          nil, 
          nil, 
          1)
        ...
        -- Set last offer type
        trade_info.last_offer_type = a_npc.trade.OFFER_SELL
      
    else
      -- Avoid flipping an item to the buy side if the stock was just depleted
      if trade_info.last_offer_type ~= a_npc.trade.OFFER_SELL then
        ...
        if #currencies > 0 then
          ...
          for buyable_item, price_info in pairs(buyable_items) do
            if buyable_item == item_name then
              ...
              --  or (trade_info.item_bought_count ~= nil and trade_info.item_bought_count <= a_npc.trade.DEDICATED_MAX_BUY_AMOUNT)))
              if trade_info.item_bought_count == nil 
                or (trade_info.item_bought_count ~= nil and trade_info.item_bought_count <= a_npc.trade.DEDICATED_MAX_BUY_AMOUNT) then
                -- Create trade offer for this item
                table.insert(offers.buy, a_npc.trade.create_offer(
                  a_npc.trade.OFFER_BUY, 
                  item_name, 
                  price_info.price, 
                ...
                -- Set last offer type
                trade_info.last_offer_type = a_npc.trade.OFFER_BUY
              else
...
function a_npc.trade.create_offer(offer_type, item, price, min_price_item_count, count)
  local result = {}
  -- Check offer type
  if offer_type == a_npc.trade.OFFER_BUY then
    ...
    local price_object = a_npc.trade.prices.table[item]
    ...
    result = {
     offer_type = offer_type,
     item = a_npc.get_item_name(item).." "..count,
     price = {[1] = price_string}
    }
  end
...
function a_npc.trade.create_custom_sell_trade_offer(option_prompt, dialogue_prompt, button_prompt, item, payments)
  return {
    offer_type = a_npc.OFFER_SELL,
...
function a_npc.trade.perform_trade(self, player_name, offer)
  ...
  if offer.offer_type == a_npc.trade.OFFER_BUY then
      ...
      for i = 1, #price_stacks do
        if inv:room_for_item("main", price_stacks[i]) then
          ...
          for j = 1, #price_stacks do
            a_npc.take_item_from_inventory_itemstring(self, price_stacks[j])
          end
          -- Add item to NPC's inventory
          a_npc.add_item_to_inventory_itemstring(self, offer.item)
    ...
    for i = 1, #price_stacks do
      ...
      if inv:contains_item("main", price_stacks[i]) then
        ...
        if inv:room_for_item("main", item_stack) then
          ...
          a_npc.take_item_from_inventory_itemstring(self, offer.item)
          -- Add price to NPC's inventory
          for i = 1, #offer.price do
            a_npc.add_item_to_inventory_itemstring(self, offer.price[i])
          end
...
minetest.register_on_player_receive_fields(function (player, formname, fields)
  ...
  if formname == "advanced_npc:trade_offer" then
    ...
    if fields then
      local player_response = a_npc.trade.results.single_trade_offer[player_name]
      -- Unlock the action timer
      a_npc.unlock_actions(player_response.npc)
      
      if fields.yes_option then
        a_npc.trade.perform_trade(player_response.npc, player_name, player_response.trade_offer)
  ...
  elseif formname == "advanced_npc:dedicated_trading_offers" then
    ...
    if fields then
      local player_response = a_npc.trade.results.trade_offers[player_name]
      -- Unlock the action timer
      a_npc.unlock_actions(player_response.npc)

      local trade_offers = a_npc.trade.results.trade_offers[player_name].offers
      -- Check which price was clicked
      for i = 1, #trade_offers do
        local price_button = "price"..tostring(i)
        if fields[price_button] then
          local trade_result = a_npc.trade.perform_trade(player_response.npc, player_name, trade_offers[i])
          if trade_result == true then
            -- Lock actions
            a_npc.lock_actions(player_response.npc)
            -- Account for buyed items
            if player_response.offers_type == a_npc.trade.OFFER_BUY then
              -- Increase the item bought count
              local offer_item_name = a_npc.get_item_name(trade_offers[i].item)
              minetest.log("Bought item name: "..dump(offer_item_name))
              minetest.log(dump(player_response.a_npc.trader_data.trade_list.both[offer_item_name]))
              -- Check if this item has been bought before
              if player_response.a_npc.trader_data.trade_list.both[offer_item_name].item_bought_count == nil then
                -- Set first count to 1
                player_response.a_npc.trader_data.trade_list.both[offer_item_name].item_bought_count = 1
              else
                -- Increase count
                player_response.a_npc.trader_data.trade_list.both[offer_item_name].item_bought_count 
                  = player_response.a_npc.trader_data.trade_list.both[offer_item_name].item_bought_count + 1
              end
            end
            -- Re-generate trade offers
            a_npc.trade.generate_trade_offers_by_status(player_response.npc)
            -- Show refreshed formspec again to player
            a_npc.trade.show_dedicated_trade_formspec(player_response.npc, player, player_response.offers_type)
            return true
          else
  ...
  elseif formname == "advanced_npc:custom_trade_offer" then
    ...
    if fields then
      local player_response = a_npc.trade.results.custom_trade_offer[player_name]
      -- Unlock the action timer
      a_npc.unlock_actions(player_response.npc)
