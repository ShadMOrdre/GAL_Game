...
--    will create node timers for each a_npc.
...
--      track of these.
--  - If a NPC has died, the spawner will choose with 50% chance to spawn a new a_npc.
--
...
a_npc.spawner.mg_villages_supported_building_types = {
    "house",
    "farm_full",
    "farm_tiny",
    "hut",
...
a_npc.spawner.replace_activated = true
a_npc.spawner.replacement_interval = 60
a_npc.spawner.spawn_delay = 10

a_npc.spawner.spawn_data = {
 status = {
    ["dead"] = 0,
    ["alive"] = 1
  }
...
function spawner.get_nodes_by_type(start_pos, end_pos, type)
  local result = {}
  local nodes = a_npc.places.find_node_in_area(start_pos, end_pos, type)
...
function spawner.scan_area(pos1, pos2)
  ...
  result.bed_type = spawner.get_nodes_by_type(start_pos, end_pos, a_npc.places.nodes.BED_TYPE)
  result.sittable_type = spawner.get_nodes_by_type(start_pos, end_pos, a_npc.places.nodes.SITTABLE_TYPE)
  result.furnace_type = spawner.get_nodes_by_type(start_pos, end_pos, a_npc.places.nodes.FURNACE_TYPE)
  result.storage_type = spawner.get_nodes_by_type(start_pos, end_pos, a_npc.places.nodes.STORAGE_TYPE)
  result.openable_type = spawner.get_nodes_by_type(start_pos, end_pos, a_npc.places.nodes.OPENABLE_TYPE)
...
function spawner.assign_places(self, pos)
  ...
  local entrance = a_npc.places.find_entrance_from_openable_nodes(doors, pos)
  if entrance then
  ...
  if entrance ~= nil and entrance.node_pos ~= nil then
    ...
    a_npc.places.add_public(self, a_npc.places.PLACE_TYPE.OPENABLE.HOME_ENTRANCE_DOOR, a_npc.places.PLACE_TYPE.OPENABLE.HOME_ENTRANCE_DOOR, entrance.node_pos)
    -- Find the position inside and outside the door
    local entrance_inside = a_npc.places.find_node_behind_door(entrance.node_pos)
    local entrance_outside = a_npc.places.find_node_in_front_of_door(entrance.node_pos)
    --minetest.set_node(entrance_inside, {name="default:apple"})
    -- Assign these places to NPC
    a_npc.places.add_public(self, a_npc.places.PLACE_TYPE.OTHER.HOME_INSIDE, a_npc.places.PLACE_TYPE.OTHER.HOME_INSIDE, entrance_inside)
    a_npc.places.add_public(self, a_npc.places.PLACE_TYPE.OTHER.HOME_OUTSIDE, a_npc.places.PLACE_TYPE.OTHER.HOME_OUTSIDE, entrance_outside)
    -- Make NPC go into their house
    --minetest.log("Place: "..dump(a_npc.places.get_by_type(self, a_npc.places.PLACE_TYPE.OTHER.HOME_INSIDE)))
    a_npc.add_task(self, a_npc.actions.cmd.WALK_TO_POS, {end_pos=a_npc.places.get_by_type(self, a_npc.places.PLACE_TYPE.OTHER.HOME_INSIDE)[1].pos, walkable={}})  
    a_npc.add_action(self, a_npc.actions.cmd.FREEZE, {freeze = false})
  end
...
-- is expired 
function a_npc.spawner.spawn_npc(pos)
  -- Get timer
  ...
  if spawned_npc_count < npc_count then
    ...
    -- Spawn a NPC
    local ent = minetest.add_entity({x=pos.x, y=pos.y+1, z=pos.z}, "a_npc:npc")
    if ent and ent:get_luaentity() then
      ent:get_luaentity().initialized = false
      a_npc.initialize(ent, pos)
      -- Assign nodes
      ...
      local entry = {
        status = a_npc.spawner.spawn_data.status.alive,
      ...
      if spawned_npc_count >= npc_count then
        ...
        minetest.log("[advanced_npc] Spawning one more NPC in "..dump(a_npc.spawner.spawn_delay).."s")
        timer:start(a_npc.spawner.spawn_delay)
      end
      return true
    else
...
function spawner.replace_mg_villages_plotmarker(pos)
  ...
  for _,value in pairs(a_npc.spawner.mg_villages_supported_building_types) do
...
if minetest.get_modpath("mg_villages") ~= nil then
  ...
  minetest.register_node("a_npc:plotmarker_auto_spawner", {
  ...
  --     if a_npc.spawner.replace_activated then
...
--[[
  ...
  minetest.register_abm({
    label = "Replace mg_villages:plotmarker with Advanced NPC auto spawners",
    nodenames = {"mg_villages:plotmarker"},
    interval = a_npc.spawner.replacement_interval,
    chance = 5,
    catch_up = true,
