...
local mummy_hp = 20
local mummy_drop = "gal:plant_papyrus"
...
spawner_DEF.on_step = function(self, dtime)
	...
	if self.timer > 1 then
		if n and n.name and n.name ~= "mobs_races:spawner_mummy" then
			self.object:remove()
		end
	end
end
...
MUMMY_DEF.on_step = function(self, dtime)
	...
	--if self.time_passed >= 5 then
	 ...
	 if current_node.name == "gal:liquid_water_source" or current_node.name == "gal:liquid_water_flowing" or current_node.name == "gal:liquid_lava_source" or current_node.name == "gal:liquid_lava_flowing" then
		--self.time_passed =  self.time_passed + dtime
...
--minetest.register_entity("mobs_races:mummy", MUMMY_DEF)
mobs:register_mob("mobs_races:mummy", MUMMY_DEF)

minetest.register_entity("mobs_races:mummy_spawner", spawner_DEF)


--spawn-egg/spawner

minetest.register_craftitem("mobs_races:spawn_egg", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		if pointed_thing.type == "node" then
			minetest.env:add_entity(pointed_thing.above,"mobs_races:mummy")
...
function mobs_races.spawn_mummy (pos, number)
	for i=0,number do
		minetest.env:add_entity(pos,"mobs_races:mummy")
	end
end

minetest.register_node("mobs_races:spawner_mummy", {
	...
	on_construct = function(pos)
		pos.y = pos.y - 0.28
		minetest.env:add_entity(pos,"mobs_races:mummy_spawner")
	end,
...
if not minetest.setting_getbool("only_peaceful_mobs") then
 minetest.register_abm({
	nodenames = {"mobs_races:spawner_mummy"},
	interval = 2.0,
	chance = 20,
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if player_near then
			if mobs < spawner_max_mobs then
				...
				minetest.env:add_entity(p,"mobs_races:mummy")
			end
		end
	end
 })
