...
local stuck_path_timeout = 2 -- how long will mob follow path before giving up
...
--print ("standing on:", nod.name, pos.y)
	...
	if nod.name and (nod.name == "default:snow" or nod.name == "gal:snow") then
		return false
	end
...
--	print ("Mob Damage is", damage)
	...
	if damage >= 1 then
		...
		if self.knock_back > 0 and tflp >= punch_interval then
	...
	if self.passive == false and self.state ~= "flop" and self.child == false and hitter:get_player_name() ~= self.owner and not mobs.invis[ name ] then
		...
		for n = 1, #objs do
			...
			if obj then
				...
				if obj.group_attack == true and obj.state ~= "attack" and obj.owner ~= name and obj.name == self.name then
...
minetest.register_entity(name, {
	...
	view_range = def.view_range or 25,
...
local c_air = minetest.get_content_id("air")
local c_ignore = minetest.get_content_id("ignore")
local c_obsidian = minetest.get_content_id("gal:stone_obsidian")
local c_brick = minetest.get_content_id("gal:stone_obsidian_brick")
local c_chest = minetest.get_content_id("lib_chests:chest_locked")
