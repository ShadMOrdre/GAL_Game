...
local function add_nametag(parent)
	if parent.npc_name then
		...
		local tag = minetest.add_entity(pos, "mobs_npc:nametag")
		if tag then
			local color = "W"
...
function mobs_npc:register_npc(name, def)
		...
		on_rightclick = function(self, clicker)
			if get_valid_entity(self) then
				...
				if player_name then
					...
					if ctrl.sneak then
						local yaw = mobs_npc:get_face_direction(self.object:getpos(), clicker:getpos())
				...
				if hitter:is_player() then
					...
					if player_name == self.owner then
						...
						if ctrl.sneak then
							...
							local v = mobs_npc:get_walk_velocity(0.1,0, yaw)
	...
	minetest.register_node(name.."_spawner", {
		...
		on_receive_fields = function(pos, formname, fields, sender)
			...
			if player_name == owner then
				...
				local luaentity = mobs_npc:spawn(npc_pos, name, {
					owner = player_name,
...
function mobs_npc:spawn(pos, name, def)
		...
		if get_valid_npc_name(def.npc_name) and index[def.npc_name] == nil then
			...
			if entity then
				...
				if luaentity then
					...
					if mobs_npc:save(luaentity) then
						local output = io.open(NPCF_DATADIR.."/index.txt", 'w')
						if output then
							output:write(minetest.serialize(index))
							io.close(output)
...
function mobs_npc:clear(npc_name)
...
function mobs_npc:load(npc_name, pos)
	if get_valid_npc_name(npc_name) then
		mobs_npc:clear(npc_name)
...
function mobs_npc:save(luaentity)
...
function mobs_npc:set_animation(luaentity, state)
...
function mobs_npc:get_index()
	return index
end

function mobs_npc:get_luaentity(npc_name)
	if get_valid_npc_name(npc_name) then
		for _,ref in pairs(minetest.luaentities) do
			if ref.object then
...
function mobs_npc:get_face_direction(v1, v2)
	if v1 and v2 then
...
function mobs_npc:get_walk_velocity(speed, y, yaw)
...
function mobs_npc:show_formspec(player_name, npc_name, formspec)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname and not fields.quit then
		...
		if npc_name ~= formname then
			local luaentity = mobs_npc:get_luaentity(npc_name)
			if luaentity then
	...
	func = function(name, param)
		...
		if cmd and npc_name and args then
			...
			elseif cmd == "load" then
				if admin or name == index[npc_name] then
					...
					if pos then
						pos.y = pos.y + 1
						if mobs_npc:load(npc_name, pos) then
		...
		if cmd and npc_name then
			...
			elseif cmd == "reload" then
				if admin or name == index[npc_name] then
					if mobs_npc:load(npc_name, nil) then
