...
mobs_npc.random_appearence = function()
	...
	local ARMOUR_SHIELD = "3d_armor_trans.png"

	local ARMOUR_HELMET_SHIELD = "3d_armor_trans.png"
	...
	local t_region = p_region[math.random(1,5)]
	local t_gender = p_gender[math.random(1,2)]
	local t_age = p_age[math.random(1,3)]
...
--
	...
	--local newTexture, newSize, newCollisionBox = mobs_npc.getVillagerAppearance(p_build[math.random(1,3)], p_region[math.random(1,5)], p_gender[math.random(1,2)], p_age[math.random(1,3)])
	local newTexture, newSize, newCollisionBox = mobs_npc.getVillagerAppearance(p_build[math.random(1,3)], t_region, t_gender, t_age)

	local newNPCName = villager_name

	--chosen_skin = skins[math.random(1, 19)]
	chosen_skin = newTexture
	--chosen_armour = armours[math.random(1, 4)]
	chosen_armour = "3d_armor_trans.png"
	chosen_weapon = weapons[math.random(1, 20)]

	local appearence_string = {chosen_skin, chosen_armour, chosen_weapon}


	return appearence_string, newSize, newCollisionBox, t_gender, t_age, newNPCName
...
local npc_make_home = function(self, dtime)
		...
		local beds = minetest.find_node_near(pos, 5, {"group:beds"})
		local light_sources = minetest.find_node_near(pos, 5, {"torch:torch", "torch:torch_wall", "torch:torch_floor", "torch:torch_ceiling", "lib_lighting:lantern", "lib_lighting:lantern_wall"})
		local doors = minetest.find_node_near(pos, 5, {"group:lib_doors"})
		...
		if beds ~= nil and light_sources ~= nil and doors ~= nil and self.home == nil and good_floor == 0 then	
			self.home = pos
...
local npc_go_home = function(self, dtime)
      ...
      if self.homepath == nil then
		 ...
		 self.homepath = pathfinder.find_path(h, h, self, dtime)
...
mobs:register_mob("mobs_npc:town_npc", {
	...
	----vBox = {-0.25,-1.00,-0.25, 0.25,0.75,0.25},
 	----vBox = {-0.20,0,-0.20, 0.20,1.8,0.20},
 	--vBox = {-0.35,0,-0.35, 0.35,1.8,0.35},
	...
	on_activate = function(self, staticdata, dtime_s)
		...
		if self.nametag and self.nametag ~= "" then
			...
			if staticdata ~= "" and staticdata ~= nil then
				...
				self.object:set_properties({textures=data[1]})
				self.object:set_properties({visual_size=data[2]})
				self.object:set_properties({collisionbox=data[3]})
				self.object:set_properties({nametag=data[4]})
				self.object:set_properties({infotext=data[5]})

				self.vName = data[6]
				self.vGender = data[7]
				self.vAge = data[8]
				self.vTexture = data[9]
				self.vSize = data[10]
				self.vBox = data[11]
...
--
				local t_texture, t_vsize, t_collision, t_gender, t_age, t_name = mobs_npc.random_appearence()
				local i_text = t_name .. " - " .. t_gender
				if t_age and t_age ~= nil then
					i_text = i_text .. " - " .. t_age
				end

				self.object:set_properties({textures=t_texture})
				--self.object:set_properties({textures=t_texture or self.vTexture})
				self.object:set_properties({visual_size=t_vsize})
				self.object:set_properties({collisionbox=t_collision})
				self.object:set_properties({nametag=t_name})
				self.object:set_properties({infotext=i_text})

				self.vName = t_name
				self.vGender = t_gender
				self.vAge = t_age
				self.vTexture = t_texture
				self.vSize = t_vsize
				self.vBox = t_collision
--
		end
	...
	get_staticdata = function(self)
		...
		local v_textures = self.textures
		local v_visual_size = self.visual_size
		local v_collisionbox = self.collisionbox
		local v_nametag = self.nametag
		local v_infotext = self.infotext

		local v_name = self.vName
		local v_gender = self.vGender
		local v_age = self.vAge
		local v_texture = self.vTexture
		local v_size = self.vSize
		local v_box = self.vBox
		--}

		local data = "" .. v_textures .. "|"
				.. v_visual_size .. "|"
				.. v_collisionbox .. "|"
				.. v_nametag .. "|"
				.. v_infotext .. "|"
