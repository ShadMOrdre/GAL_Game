...
local function getformspec(pos)
  ...
  spawnerformspec = spawnerformspec .. "field[.5,0.5;5,1;entity;Mob specifier (e.g. mobs:sheep);"..meta:get_string("entity").."]"
...
minetest.register_node(":mobs:spawner",{
	...
	on_punch = function (pos, node, puncher, pointed_thing)
		print("punched")
		if minetest.check_player_privs(puncher:get_player_name(),{ server=true }) then
			local meta = minetest.get_meta(pos)
			local entity = meta:get_string("entity")
			if entity ~= "" then	
				--split by commas
				--check that entities are registered LuaObjects
				...
				minetest.add_entity(spawnpos,entity)
			else
...
minetest.register_abm({
	...
	if entity ~= "" then	
		...
		spawnpos.y = spawnpos.y + 3
		minetest.log("action","Spawn block spawning "..tostring(entity).." at "..minetest.pos_to_string(spawnpos))
		local r = minetest.add_entity(spawnpos,entity)
		if r == -1 then
