...
minetest.register_node("cottages:water_gen", {
	...
	on_construct = function(pos)
		...
		meta:set_string("formspec",
			...
			"button_exit[6.0,0.0;2,0.5;public;".."Public?".."]"..
		...
		inv:set_size('main', 6)
		meta:set_string("infotext", "Public tree trunk well") -- (punch with empty bucket to fill bucket)")
	end,
	after_place_node = function(pos, placer)
		local meta = minetest.get_meta(pos)
		meta:set_string("owner", placer:get_player_name() or "")
		meta:set_string("infotext", "Public tree trunk well (owned by %s) " .. format(meta:get_string("owner")))
		-- no bucket loaded
	...
	on_punch = function(pos, node, puncher)
		...
		if( name ~= owner and public~="public") then
			minetest.chat_send_player( name, "This tree trunk well is owned by %s. You can't use it. " .. format(name))
			return
		end
		...
		if( bucket and bucket~="") then
			if( not(pinv:room_for_item("main", bucket))) then
				minetest.chat_send_player( puncher:get_player_name(),
					"Sorry. You have no room for the bucket. Please free some "..
