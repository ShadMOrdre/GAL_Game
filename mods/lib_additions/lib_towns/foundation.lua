...
--
function lib_towns.ground(pos) -- role model: Wendelsteinkircherl, Brannenburg
  local p2 = pos
  local cnt = 0
  local mat = "dirt"
  p2.y = p2.y-1
  while true do
    ...
    minetest.swap_node(p2, {name="gal:"..mat})
    p2.y = p2.y-1
  end
end
--
-- function to fill empty space below baseplate when building on a hill
--
function lib_towns.foundation(pos, width, depth, height, rotation)
  local p5 = lib_towns.shallowCopy(pos)
    ...
    for xi = 0,fwidth-1 do
      for zi = 0,fdepth-1 do
        if yi == 0 then
          ...
          minetest.after(1,lib_towns.ground,p)--(p)
        else
