...
function lib_tools.crafting.add_craft(output, recipe)
	local output_name = string.gsub(output, ":", "_", 1)
	lib_tools.crafts[output_name] = recipe
end
...
local function check_craft(pos, list)
	...
	local wear = 0
	local x = {}
	for i,v in pairs(lib_tools.crafts) do
...
local function dfurnace_node_timer(pos, elapsed)
	...
	if fuel_time <= fuel_totaltime and fuel_totaltime ~= 0 then
		swap_node(pos, "lib_tools:dual_furnace_active")
		result = true
		...
		swap_node(pos, "lib_tools:dual_furnace_inactive")
...
for _, nn in pairs({"active", "inactive"}) do
	...
	minetest.register_node("lib_tools:dual_furnace_" .. nn, {
		...
		paramtype = "light",
		drop = "lib_tools:dual_furnace_inactive",
...
minetest.register_craft({
	output = "lib_tools:dual_furnace_inactive",
	recipe = {
		{"", "default:steel_ingot", ""},
...
lib_tools.crafting.add_craft("default:bronze_ingot", {
