	...
	if obj_count > 0 then
		...
		for i = 1, shown_items do
			...
			if not list[i]:is_empty() then
				...
				local ent = minetest.add_entity(obj_pos, "lib_tools:shelf_item")
				ent:set_properties({
...
function lib_tools.register_shelf(name, def)
	...
	minetest.register_node("lib_tools:"..name, {
		...
		on_blast = function(pos)
			local drops = {}
			default.get_inventory_drops(pos, "itemshelves:shelf", drops)
			drops[#drops + 1] = "itemshelves:shelf"
			minetest.remove_node(pos)
			return drops
		end
	})
...
-- Entity for shelf
minetest.register_entity("lib_tools:shelf_item", {
	hp_max = 1,
	visual = "wielditem",
