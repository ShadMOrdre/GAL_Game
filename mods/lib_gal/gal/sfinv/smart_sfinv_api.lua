...
gal.sfinv.smart_sfinv_api.registered_enhancements = {}
...
function gal.sfinv.smart_sfinv_api.register_enhancement(def)
	table.insert(gal.sfinv.smart_sfinv_api.registered_enhancements, def)
end
...
gal.sfinv.smart_sfinv_api.defaults = enh_handler_class
enh_handler_class_meta = {__index = enh_handler_class }

function enh_handler_class:run_enhancements(enh_method, ...)
	for _, enh in ipairs(gal.sfinv.smart_sfinv_api.registered_enhancements) do
...
----------------------------------------------
function gal.sfinv.smart_sfinv_api.get_handler( context )
...
----------------------------------------------
-- Redefnition gal.sfinv.get_nav_fs
----------------------------------------------
local orig_get_nav_fs = gal.sfinv.get_nav_fs
function gal.sfinv.get_nav_fs(player, context, nav, current_idx)
	...
	local handler = gal.sfinv.smart_sfinv_api.get_handler( context )
...
----------------------------------------------
-- Redefinition gal.sfinv.make_formspec
----------------------------------------------
local orig_make_formspec = gal.sfinv.make_formspec
function gal.sfinv.make_formspec(player, context, content, show_inv, size)
	...
	local nav_fs = gal.sfinv.get_nav_fs(player, context, context.nav_titles, context.nav_idx)
...
--print(_dummy_page, "Size:", enh_handler_class.formspec_size, "Theme:" ,enh_handler_class.theme_main, "Inv:", enh_handler_class.theme_inv)
