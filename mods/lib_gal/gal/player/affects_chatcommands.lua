...
minetest.register_chatcommand("affect",{
	...
	func = function (name, param)
		...
		if ( gal.player.affects.affectPlayer(aname,affectid) ) then
			minetest.chat_send_player(name,aname.." has been affected by "..gal.player.affects._affects[affectid].name)
		else
...
minetest.register_chatcommand("removeaffect",{
	...
	func = function (name, param)
		local aname, affectid = string.match(param, "([^ ]+) (.+)")	
		if ( gal.player.affects.removeAffect(aname,affectid) ) then
