...
-- global values
gal.player.hud.registered_items = {}
gal.player.hud.damage_events = {}
gal.player.hud.breath_events = {}
...
local items = gal.player.hud.registered_items
...
function gal.player.hud.register(name, def)
		...
		for _,v in pairs(def.events) do
			if v and v.type and v.func then
				if v.type == "damage" then
					table.insert(gal.player.hud.damage_events, v)
				end

				if v.type == "breath" then
					table.insert(gal.player.hud.breath_events, v)
				end
			end
		end
	end
...
function gal.player.hud.swap_statbar(player, item1, item2)
...
function gal.player.hud.change_item(player, name, def)
	...
	if def.offset and elem.offset then
		if def.item_name and def.offset == "item" then
			...
			if def.item_name then
				gal.player.hud.swap_statbar(player, name, def.item_name)
			end
		else
...
function gal.player.hud.remove_item(player, name)
...
function gal.player.hud.player_event(player, event)
	...
	minetest.after(0, function(player) -- ADDED (player)
		if event == "health_changed" then
			for _,v in pairs(gal.player.hud.damage_events) do
				if v.func then
					v.func(player)
				end
			end
		...
		if event == "breath_changed" then
			for _,v in pairs(gal.player.hud.breath_events) do
				if v.func then
					v.func(player)
				end
			end
...
core.register_playerevent(gal.player.hud.player_event)
