...
if minetest.get_modpath("intllib") then
	armor_i18n.gettext = intllib.Getter()
else
	-- S = function(s) return s end
	-- internationalization boilerplate
	armor_i18n.gettext, armor_i18n.ngettext = dofile(gal.path_mod.."/intllib.lua")
end
...
gal.player.armor = {
	...
	materials = {
		cotton = nil,						--**Used for clothing.       {lib_ecology_plant_cotton, lib_textiles:cotton}
		wool = nil,						--**Used for clothing.       {mobs_redo:wool, lib_textiles:wool}
		bone = nil,						--**Used for clothing/gal.player.armor.       {mobs_redo:bone}
		leather = nil,						--**Used for clothing/gal.player.armor.       {mobs_redo:leather, lib_textiles:leather}
		leather_black = nil,
		leather_brown = nil,
		leather_grey = nil,
		leather_dark_grey = nil,
		leather_dark_green = nil,
		studded = nil,
		ninja = nil,
		copper = nil,
		brass = nil,
		...
		titanium = nil,
		wood = nil,						--"group:wood",
		cactus = nil,						--"default:cactus",
		steel = nil,						--"default:steel_ingot",
		bronze = nil,						--"default:bronze_ingot",
		diamond = nil,						--"default:diamond",
		gold = nil,						--"default:gold_ingot",
		mithril = nil,						--"moreores:mithril_ingot",
		crystal = nil,						--"ethereal:crystal_ingot",
		admin2 = nil,
		gold2 = nil,
		mithril2 = nil,
		diamond2 = nil,
	...
	fire_nodes = {
		{"gal:liquid_lava_cooling_flowing",            5, 8},
		{"gal:liquid_lava_cooling_source",            5, 8},
		{"gal:liquid_lava_flowing",            5, 8},
		{"gal:liquid_lava_source",            5, 8},
		{"gal:fire_flame_basic",           3, 4},
		{"gal:fire_flame_permanent",            3, 4},
		{"torch:torch",           1, 1},
		{"torch:torch_ceiling",   1, 1},
		{"torch:torch_wall",      1, 1},
...
gal.player.armor.register_armor = function(self, name, def)
	minetest.register_tool(name, def)
end

gal.player.armor.register_armor_group = function(self, group, base)
	base = base or 100
...
gal.player.armor.register_on_update = function(self, func)
	if type(func) == "function" then
		table.insert(self.registered_callbacks.on_update, func)
	end
end

gal.player.armor.register_on_equip = function(self, func)
	if type(func) == "function" then
		table.insert(self.registered_callbacks.on_equip, func)
	end
end

gal.player.armor.register_on_unequip = function(self, func)
	if type(func) == "function" then
		table.insert(self.registered_callbacks.on_unequip, func)
	end
end

gal.player.armor.register_on_damage = function(self, func)
	if type(func) == "function" then
		table.insert(self.registered_callbacks.on_damage, func)
	end
end

gal.player.armor.register_on_destroy = function(self, func)
	if type(func) == "function" then
		table.insert(self.registered_callbacks.on_destroy, func)
	end
end

gal.player.armor.run_callbacks = function(self, callback, player, index, stack)
	if stack then
...
gal.player.armor.update_player_skin = function(self, player, skin_texture)
	if not player then
		return
	end
	local name = player:get_player_name()
	if self.textures[name] then
		gal.player.api.set_textures(player, {
			self.textures[name].skin_texture,
			--self.textures[name].cape,
...
gal.player.armor.update_player_visuals = function(self, player)
	if not player then
		return
	end
	local name = player:get_player_name()
	if self.textures[name] then
		gal.player.api.set_textures(player, {
			player:get_properties().textures[1] or appearance_get_skin(name) or self.textures[name].skin,
			--self.textures[name].cape,
			self.textures[name].armor,
			self.textures[name].wielditem,
		})
...
gal.player.armor.set_player_armor = function(self, player)
	...
	local preview = gal.player.armor:get_preview(name)
	...
	for group, level in pairs(levels) do
		if level > 0 then
			level = level * gal.player.armor.config.level_multiplier
	...
	if use_armor_monoid then
		armor_monoid.monoid:add_change(player, change, "lib_armor:armor")
	else
	...
	elseif use_pova_mod then
		...
		pova.add_override(name, "lib_armor", {
...
gal.player.armor.punch = function(self, player, hitter, time_from_last_punch, tool_capabilities)
...
gal.player.armor.damage = function(self, player, index, stack, use)
...
gal.player.armor.get_player_skin = function(self, name)
	...
	elseif self.skin_mod == "appearance" then
		return appearance_get_skin(name)		--appearance_update(name)		--or player:get_properties().textures[1]		--appearance_get_skin(name)
	else
		
	end
	return gal.player.armor.default_skin..".png"
end

gal.player.armor.add_preview = function(self, preview)
...
gal.player.armor.get_preview = function(self, name)
	if self.skin_mod == "appearance" then
		...
		local preview = string.gsub(gal.player.armor:get_player_skin(name), ".png", "_preview.png")
		if skin_previews[preview] then
			return preview
		end
	end
...
gal.player.armor.get_armor_formspec = function(self, name, listring)
	if gal.player.armor.def[name].init_time == 0 then
	...
	formspec = formspec:gsub("armor_preview", gal.player.armor.textures[name].preview)
	formspec = formspec:gsub("armor_level", gal.player.armor.def[name].level)
	for _, attr in pairs(self.attributes) do
		formspec = formspec:gsub("armor_attr_"..attr, gal.player.armor.def[name][attr])
	end
	for group, _ in pairs(self.registered_groups) do
		formspec = formspec:gsub("armor_group_"..group,
			gal.player.armor.def[name].groups[group])
	end
	return formspec
end

gal.player.armor.get_element = function(self, item_name)
	for _, element in pairs(gal.player.armor.elements) do
...
gal.player.armor.serialize_inventory_list = function(self, list)
	local list_table = {}
...
gal.player.armor.deserialize_inventory_list = function(self, list_string)
...
gal.player.armor.load_armor_inventory = function(self, player)
...
gal.player.armor.save_armor_inventory = function(self, player)
...
gal.player.armor.update_inventory = function(self, player)
	-- DEPRECATED: Legacy inventory support
end

gal.player.armor.set_inventory_stack = function(self, player, i, stack)
...
gal.player.armor.get_valid_player = function(self, player, msg)
	msg = msg or ""
	if not player then
...
gal.player.armor.drop_armor = function(pos, stack)
