...
-- read/write
function gal.player.hunger.read(player)
...
function gal.player.hunger.save(player)
	local inv = player:get_inventory()
	local name = player:get_player_name()
	local value = gal.player.hunger.players[name].lvl
...
function gal.player.hunger.update_hunger(player, new_lvl)
	...
	gal.player.hunger.players[name].lvl = lvl
	if lvl > 20 then
		lvl = 20
	end
	gal.player.hud.change_item(player, "hunger", {number = lvl})
	gal.player.hunger.save(player)
end
local update_hunger = gal.player.hunger.update_hunger

-- player-action based hunger changes
function gal.player.hunger.handle_node_actions(pos, oldnode, player, ext)
	if not player or not player:is_player() then
		return
	end
	local name = player:get_player_name()
	if not name or not gal.player.hunger.players[name] then
		return
	end

	local exhaus = gal.player.hunger.players[name].exhaus
	if not exhaus then
	...
	if exhaus > HUNGER_EXHAUST_LVL then
		exhaus = 0
		local h = tonumber(gal.player.hunger.players[name].lvl)
		if h > 0 then
			update_hunger(player, h - 1)
		end
	end

	gal.player.hunger.players[name].exhaus = exhaus
end
...
function set_sprinting(name, sprinting)
	...
	local def = {}
	if armor_mod and gal.player.armor and gal.player.armor.def[name] then
		def = gal.player.armor.def[name]
	end
...
local function hunger_globaltimer(dtime)
	...
	if action_timer > HUNGER_MOVE_TICK then
		for _,player in ipairs(minetest.get_connected_players()) do
			...
			if controls.up or controls.down or controls.left or controls.right then
				gal.player.hunger.handle_node_actions(nil, nil, player)
			end
			...
			if enable_sprint then
				...
				and gal.player.hunger.players[name].lvl > 6 then
					...
					update_hunger(player,
						gal.player.hunger.players[name].lvl - (SPRINT_DRAIN * HUNGER_MOVE_TICK))
				else
	...
	if hunger_timer > HUNGER_TICK then
		for _,player in ipairs(minetest.get_connected_players()) do
			local name = player:get_player_name()
			local tab = gal.player.hunger.players[name]
			if tab then
...
-- food functions
local food = gal.player.hunger.food

function gal.player.hunger.register_food(name, hunger_change, replace_with_item, poisen, heal, sound)
	food[name] = {}
...
local function poisenp(tick, time, time_left, player)
	...
	if time_left < time then
		...
		gal.player.hud.change_item(player, "hunger", {text = "hud_hunger_fg.png"})
	end
...
core.do_item_eat = function(hp_change, replace_with_item, itemstack, user, pointed_thing)
	...
	itemstack = gal.player.hunger.eat(hp_change, replace_with_item, itemstack, user, pointed_thing)
...
function gal.player.hunger.eat(hp_change, replace_with_item, itemstack, user, pointed_thing)
	...
	local func = gal.player.hunger.item_eat(def.saturation, def.replace, def.poisen, def.healing, def.sound)
	return func(itemstack, user, pointed_thing)
end

function gal.player.hunger.item_eat(hunger_change, replace_with_item, poisen, heal, sound)
	return function(itemstack, user, pointed_thing)
		...
		local sat = tonumber(gal.player.hunger.players[name].lvl or 0)
		local hp = user:get_hp()
		-- Saturation
		if sat < HUNGER_MAX and hunger_change then
			sat = sat + hunger_change
			gal.player.hunger.update_hunger(user, sat)
		end
		-- Healing
		...
		-- Poison
		if poisen then
			gal.player.hud.change_item(user, "hunger", {text = "hunger_statbar_poisen.png"})
