...
MAGICMISSLE_ENTITY.on_step = function(self, dtime)
	...
	--if self.timer>0.2 then
		...
		for k, obj in pairs(objs) do
			if obj:get_luaentity() ~= nil then
				if obj:get_luaentity().name ~= "gal:magicmissle" and obj:get_luaentity().name ~= "__builtin:item" then
					minetest.sound_play("magic_magicmissle_hit",{object=self.object})
					local skill = gal.player.skills.get_skill(hitter:get_player_name(),SKILL_MAGIC)
					local damage = self.max_damage * ( (skill.level) / gal.player.skills.get_def(SKILL_MAGIC)['max_level'] )
	...
	if self.lastpos.x~=nil then
		--													 and node.name ~= "default:water_source" and node.name ~= "default:water_flowing"
		if ( minetest.registered_nodes[node.name].walkable == true or minetest.registered_nodes[node.name].walkable == nil ) and ( not node.groups.liquids ) and ( node.name ~= "air" ) then
			minetest.sound_play("magic_magicmissle_hit",{object=self.object})
			hitparticles(pos)
			self.object:remove()
		end
...
minetest.register_entity("gal:magicmissle", MAGICMISSLE_ENTITY)
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		if sk.level >= 4 then		
			...
			if p_mana >= mana then
				...
				local obj = minetest.add_entity({x=playerpos.x,y=playerpos.y+1.5,z=playerpos.z}, "lib_player:magicmissle")
				...
				gal.player.magic.update_magic(user,name)
			else
...
gal.player.magic.register_spell(magicmissle_spell)
