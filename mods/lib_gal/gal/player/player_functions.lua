...
gal.player.pl_hooks = {}
function gal.player.player_loop(dtime)
	local p = minetest.get_connected_players()
	local reset_hooks = {}
	for k,hook in pairs(gal.player.pl_hooks) do
		gal.player.pl_hooks[k].timer = gal.player.pl_hooks[k].timer + dtime
		if gal.player.pl_hooks[k].timer >= gal.player.pl_hooks[k].timeout then
			for _, player in pairs(p) do
				local name = player:get_player_name()
				gal.player.pl_hooks[k].timer = 0
				gal.player.pl_hooks[k].func(player,name,dtime)
			end
		end 
	end
end

function gal.player.register_pl_hook(f,t)
	table.insert(gal.player.pl_hooks,{func=f,timeout=t,timer=0})
end

function gal.player.teleport(player,pos)
	local name = player:get_player_name();
	gal.player.data.set(name,"lastpos",pos)
	player:moveto(pos)
end

function gal.player.check_spawn(player)
	print("checking spawn")
	local count = 0
	local newpos = player:getpos()
	local badpos = false
	if gal.player.obj_stuck(player) == true then
		...
		local ab = gal.player.above_ground(newpos)
		...
		gal.player.teleport(player,newpos)
		--count = count + 1
		--badpos = true
	...
	gal.player.data.set(n,"homepos",newpos)
	player:hud_remove(gal.player.data.get(n,"spawning_hud"))
	gal.player.data.set(n,"spawning_hud",nil)
end

-- sees if a player  or entity is in a block
function gal.player.obj_stuck(obj)
	local pos = obj:getpos()
	local pn = gal.player.get_obj_nodes(obj)
...
function gal.player.get_obj_nodes(obj)
	local pos = obj:getpos()
	local retval = {}
...
function gal.player.above_ground(pos)
	local step = 0
	local dest = 0
	if pos.y > 0 then
		step = -1
...
function hunger_join_player(player)
	local name = player:get_player_name()		
	local lvl = gal.player.data.get_number(name,"hunger_lvl")
	if lvl > 20 then
		lvl = 20
	end
	minetest.after(0.8, function()
		gal.player.hud.change_item(player, "hunger", {offset = "item", item_name = "hunger"})
		gal.player.hud.change_item(player, "hunger", {number = lvl, max = 20})
	end)
end

function gal.player.is_night()
