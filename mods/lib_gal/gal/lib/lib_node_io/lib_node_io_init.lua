...
local MP = gal.lib.node_io.path



-- get target side

gal.lib.node_io.get_target_side = function(pos, target_pos)
...
gal.lib.node_io.get_pointed_side = function(pointer, pointed_thing)
...
gal.lib.node_io.update_neighbors = function(pos)
	local p, n, d
...
gal.lib.node_io.can_put_item = function(pos, node, side)
...
gal.lib.node_io.room_for_item = function(pos, node, side, itemstack, count) -- returns non-negative number
	local ndef = minetest.registered_nodes[node.name]
	if not ndef.node_io_room_for_item then return 0 end
	return ndef.node_io_room_for_item(pos, node, side, itemstack, count)
end
gal.lib.node_io.room_for_liquid = function(pos, node, side, liquid, millibuckets) -- returns non-negative number
	local ndef = minetest.registered_nodes[node.name]
...
gal.lib.node_io.put_item = function(pos, node, side, putter, itemstack) -- returns itemstack with leftovers or a cleared itemstack
	local ndef = minetest.registered_nodes[node.name]
	if not ndef.node_io_put_item then return itemstack end
	return ndef.node_io_put_item(pos, node, side, putter, itemstack)
end
gal.lib.node_io.put_liquid = function(pos, node, side, putter, liquid, millibuckets) -- returns leftover millibuckets or zero
	local ndef = minetest.registered_nodes[node.name]
...
gal.lib.node_io.take_item = function(pos, node, side, taker, want_item, want_count) -- returns itemstack or nil
	local ndef = minetest.registered_nodes[node.name]
	if not ndef.node_io_take_item then return nil end
	return ndef.node_io_take_item(pos, node, side, taker, want_item, want_count)
end
gal.lib.node_io.take_liquid = function(pos, node, side, taker, want_liquid, want_count) -- returns {name:string, millibuckets:int} or nil
	local ndef = minetest.registered_nodes[node.name]
...
gal.lib.node_io.get_inventory = function(pos)
	if pos.type then
...
gal.lib.node_io.compare_itemstack = function(itemstack1, itemstack2)
...
gal.lib.node_io.room_for_item_in_inventory = function(inv, inv_name, itemstack, count)
	...
	for i = 1, inv:get_size(inv_name) do
		...
		if gal.lib.node_io.compare_itemstack(stack, itemstack) then
...
gal.lib.node_io.get_inventory_size = function(pos, inv_name)
	local inv = gal.lib.node_io.get_inventory(pos) -- see gal.lib.node_io.get_inventory() for pos details
	if not inv then return 0 end
	return inv:get_size(inv_name)
end

gal.lib.node_io.get_inventory_name = function(pos, inv_name, index)
	local inv = gal.lib.node_io.get_inventory(pos) -- see gal.lib.node_io.get_inventory() for pos details
	if not inv or index < 1 or index > inv:get_size(inv_name) then return "" end
	return inv:get_stack(inv_name, index):get_name()
end

gal.lib.node_io.get_inventory_stack = function(pos, inv_name, index)
	local inv = gal.lib.node_io.get_inventory(pos) -- see gal.lib.node_io.get_inventory() for pos details
...
gal.lib.node_io.put_item_in_inventory = function(pos, node, inv_name, putter, itemstack)
	local inv = gal.lib.node_io.get_inventory(pos) -- see gal.lib.node_io.get_inventory() for pos details
...
gal.lib.node_io.take_item_from_inventory = function(pos, node, inv_name, taker, want_item, want_count)
	local inv = gal.lib.node_io.get_inventory(pos) -- see gal.lib.node_io.get_inventory() for pos details
	...
	for i = 1, inv:get_size(inv_name) do
		...
		if stack_item ~= "" and (want_item == nil or (type(want_item) == "userdata" and gal.lib.node_io.compare_itemstack(stack, want_item)) or stack_item == want_item) then
			local result_stack
...
gal.lib.node_io.init_main_inventory = function(node_name, allow_take)
	local def = {}
	...
	def.node_io_room_for_item = function(pos, node, side, itemstack, count)
		local inv = gal.lib.node_io.get_inventory(pos) -- see gal.lib.node_io.get_inventory() for pos details
		if not inv then return 0 end
		return gal.lib.node_io.room_for_item_in_inventory(inv, "main", itemstack, count)
	end
	def.node_io_put_item = function(pos, node, side, putter, itemstack)
		return gal.lib.node_io.put_item_in_inventory(pos, node, "main", putter, itemstack)
	end

	if allow_take then
		def.node_io_can_take_item = function(pos, node, side) return true end
		def.node_io_get_item_size = function(pos, node, side)
			return gal.lib.node_io.get_inventory_size(pos, "main")
		end
		def.node_io_get_item_name = function(pos, node, side, index)
			return gal.lib.node_io.get_inventory_name(pos, "main", index)
		end
		def.node_io_get_item_stack = function(pos, node, side, index)
			return gal.lib.node_io.get_inventory_stack(pos, "main", index)
		end
		def.node_io_take_item = function(pos, node, side, taker, want_item, want_count)
			return gal.lib.node_io.take_item_from_inventory(pos, node, "main", taker, want_item, want_count)
		end
	end
...
gal.lib.node_io.init_furnace = function(node_name)
	local def = {}
	...
	def.node_io_room_for_item = function(pos, node, side, itemstack, count)
		...
		return gal.lib.node_io.room_for_item_in_inventory(inv, inv_name, itemstack, count)
	end
	def.node_io_put_item = function(pos, node, side, putter, itemstack)
		local inv_name
		if side == "U" then inv_name = "src" elseif side == "D" then inv_name = "dst" else inv_name = "fuel" end
		return gal.lib.node_io.put_item_in_inventory(pos, node, inv_name, putter, itemstack)
	end
	...
	def.node_io_get_item_size = function(pos, node, side)
		local inv_name
		if side == "U" then inv_name = "src" elseif side == "D" then inv_name = "dst" else inv_name = "fuel" end
		return gal.lib.node_io.get_inventory_size(pos, inv_name)
	end
	def.node_io_get_item_name = function(pos, node, side, index)
		local inv_name
		if side == "U" then inv_name = "src" elseif side == "D" then inv_name = "dst" else inv_name = "fuel" end
		return gal.lib.node_io.get_inventory_name(pos, inv_name, index)
	end
	def.node_io_get_item_stack = function(pos, node, side, index)
		local inv_name
		if side == "U" then inv_name = "src" elseif side == "D" then inv_name = "dst" else inv_name = "fuel" end
		return gal.lib.node_io.get_inventory_stack(pos, inv_name, index)
	end
	def.node_io_take_item = function(pos, node, side, taker, want_item, want_count)
		local inv_name
		if side == "U" then inv_name = "src" elseif side == "D" then inv_name = "dst" else inv_name = "fuel" end
		return gal.lib.node_io.take_item_from_inventory(pos, node, inv_name, taker, want_item, want_count)
	end
...
if minetest.global_exists("default") then
	dofile(MP.."lib_node_io_support.lua")
end
