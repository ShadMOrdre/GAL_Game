...
local nodeiodef = {
	...
	node_io_put_liquid = function(pos, node, side, putter, liquid, millibuckets)
		...
		for buffer,data in pairs(buffers) do
			...
			local didnt_fit = gal.lib.fluid.insert_into_buffer(pos, buffer, liquid, millibuckets)
	...
	node_io_room_for_liquid = function(pos, node, side, liquid, millibuckets)
		...
		local insertable = 0
		for buffer,data in pairs(buffers) do
			local insert = gal.lib.fluid.can_insert_into_buffer(pos, buffer, liquid, millibuckets)
			if insert > 0 then
				insertable = insert
				break
			end
	...
	node_io_take_liquid = function(pos, node, side, taker, want_liquid, want_millibuckets)
		...
		for buffer,data in pairs(buffers) do
			...
			if (fluid == want_liquid or want_liquid == "") and storage >= want_millibuckets then
				name, took = gal.lib.fluid.take_from_buffer(pos, buffer, want_millibuckets)
	...
	node_io_get_liquid_size = function (pos, node, side)
		...
		local cnt = 0
		local bfs = gal.lib.fluid.get_node_buffers(pos)
...
function gal.lib.fluid.register_node(nodename)
