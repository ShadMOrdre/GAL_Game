...
function gal.lib.fluid.get_node_buffers(pos)
...
function gal.lib.fluid.get_buffer_data(pos, buffer)
	local node, nodedef = node_data(pos)
	local buffers = gal.lib.fluid.get_node_buffers(pos)
...
function gal.lib.fluid.buffer_accepts_fluid(pos, buffer, fluid)
	local bfdata = gal.lib.fluid.get_buffer_data(pos, buffer)
...
function gal.lib.fluid.can_insert_into_buffer(pos, buffer, fluid, count)
	local bfdata = gal.lib.fluid.get_buffer_data(pos, buffer)
	if not bfdata then return 0 end
	if not gal.lib.fluid.buffer_accepts_fluid(pos, buffer, fluid) then return 0 end
...
function gal.lib.fluid.insert_into_buffer(pos, buffer, fluid, count)
	local bfdata = gal.lib.fluid.get_buffer_data(pos, buffer)
	if not bfdata then return count end
	if bfdata.fluid ~= fluid and bfdata.fluid ~= "" then return count end

	local can_put = gal.lib.fluid.can_insert_into_buffer(pos, buffer, fluid, count)
...
function gal.lib.fluid.can_take_from_buffer(pos, buffer, count)
	local bfdata = gal.lib.fluid.get_buffer_data(pos, buffer)
...
function gal.lib.fluid.take_from_buffer(pos, buffer, count)
	local bfdata = gal.lib.fluid.get_buffer_data(pos, buffer)
	...
	local take_count = gal.lib.fluid.can_take_from_buffer(pos, buffer, count)
...
function gal.lib.fluid.buffer_to_string(buffer)
	if not buffer then return "" end
	local amount      = gal.lib.fluid.comma_value(buffer.amount)
	local capacity    = gal.lib.fluid.comma_value(buffer.capacity)
	local description = "Empty"

	if buffer.fluid ~= "" then
		description = gal.lib.fluid.cleanse_node_description(buffer.fluid)
	end

	return ("%s (%s / %s %s)"):format(description, amount, capacity, gal.lib.fluid.unit)
end
