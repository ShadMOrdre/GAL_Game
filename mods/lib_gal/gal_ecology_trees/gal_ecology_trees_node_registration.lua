...
local S = gal_ecology_trees.intllib
...
for i, stone in ipairs(gal.lib.csv.read("|", gal_ecology_trees.path .. "/" .. gal_ecology_trees.nodes_data_file .. ".csv")) do
...
--USE MAX LIGHT FOR CROPS
		...
		new_node_def.on_place = function(itemstack, placer, pointed_thing)
			...
			if def and def.liquidtype == "source" and
				...
				if not minetest.is_protected(pos, player_name) then
					minetest.set_node(pos, {name = "gal:"..node_name, param2 = math.random(0, 3)})
		...
		if string.find(groups, "sea") then
			if string.find(tile_string, "_coral") then
				new_node_def.on_place = function(itemstack, placer, pointed_thing)
					...
					if minetest.get_node(pos_under).name ~= "gal:coral_skeleton" or (string.find(minetest.get_node(pos_above).name, "water_") and string.find(minetest.get_node(pos_above).name, "_source")) then
						return itemstack
					end
					...
					minetest.set_node(pos_under, {name = "gal:"..node_name..""})
				...
				new_node_def.after_destruct  = function(pos, oldnode)
					minetest.set_node(pos, {name = "gal:coral_skeleton"})
				end
			else
				new_node_def.on_place = function(itemstack, placer, pointed_thing)
					if pointed_thing.type ~= "node" or not placer then
	...
	if string.find(node_name, "_leaves") then
		if string.find(node_name, "tree_") then
			...
			new_node_def.on_timer = function(pos)
				...
				for _, item in ipairs(drops) do
					local is_leaf
					for _, v in pairs({"gal:"..node_name..""}) do
						if v == item then
							is_leaf = true
						end
					end
			...
			new_node_def.on_timer = function(pos)
				if minetest.find_node_near(pos, RADIUS, "gal:tree_"..node_name.."_trunk") then
					return false
				end
				local node = minetest.get_node(pos)
				local drops = minetest.get_node_drops(node.name)
	...
	if string.find(node_name, "_trunk") then
		new_node_def.after_destruct = function(pos, oldnode)
			...
			for _, v in pairs(minetest.find_nodes_in_area(vector.subtract(pos, RADIUS),
					vector.add(pos, RADIUS), "gal:"..node_name.."")) do
	...
	if string.find(node_name, "_sapling") then
		...
		new_node_def.on_timer = function(pos)
			if not gal_ecology_trees.can_grow(pos) then
			...
			if string.find(grow, ",") then
				...
				minetest.place_schematic(pos, gal_ecology_trees.schematics.select(rname), "random", nil, true, "place_center_x, place_center_z")
			end
		end
		new_node_def.on_place = function(itemstack, placer, pointed_thing)
			itemstack = gal_ecology_trees.sapling_on_place(itemstack, placer, pointed_thing,
				"gal:" .. node_name .. "",
...
--##REGISTER SHAPES
	...
	if (string.find(node_name, "item_")) and (not (string.find(node_name, "_bale") or string.find(node_name, "_mat"))) then
		if string.find(node_name, "reed") or string.find(node_name, "reet") or string.find(node_name, "hay") or string.find(node_name, "straw") then
			...
			gal.lib.shapes.register_node(""..node_name.."", "stairs_outer")
			gal.lib.shapes.register_node(""..node_name.."", "slab")
			--gal.lib.shapes.register_node(""..node_name.."", "step")
			--gal.lib.shapes.register_node(""..node_name.."", "ceiling")
			gal.lib.shapes.register_node(""..node_name.."", "pane")
			gal.lib.shapes.register_node(""..node_name.."", "pane_centered")
			gal.lib.shapes.register_node(""..node_name.."", "wall")
