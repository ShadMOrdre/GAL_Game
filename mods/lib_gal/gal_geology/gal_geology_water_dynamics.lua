...
gal_geology.waterplus={}

-- Settings
gal_geology.waterplus.finite_water_steps=32 --how many finite water steps
gal_geology.waterplus.finite_water_inc_skip=1  --how many waters to skip before inc


gal_geology.waterplus.base_fluid = "gal:liquid_water_rushing_source"
gal_geology.waterplus.base_fluid_flowing = "gal:liquid_water_rushing_flowing"


-- Setup Finite
gal_geology.waterplus.finite_water_inc=1/(gal_geology.waterplus.finite_water_steps /(1+gal_geology.waterplus.finite_water_inc_skip))
gal_geology.waterplus.finite_water_max=math.floor(1.43/gal_geology.waterplus.finite_water_inc) --how many finite water values (give a new style water effect)
...
dPrint("Water steps: "..gal_geology.waterplus.finite_water_steps)
dPrint("Water max: "..gal_geology.waterplus.finite_water_max)
dPrint("Water inc: "..gal_geology.waterplus.finite_water_inc)
dPrint("Water inc_skip: "..gal_geology.waterplus.finite_water_inc_skip)

-- Locals
local h=gal_geology.waterplus.finite_water_inc
local c=1
...
gal_geology.waterplus.finite_blocks = {}
gal_geology.waterplus.register_step = function(a,height)
	...
	minetest.register_node(":gal:liquid_finite_"..a, {
		...
		groups = {water=3,finite_water=((a/gal_geology.waterplus.finite_water_steps)*100), puts_out_fire=1},
		node_box = {
			type="fixed",
			fixed={
...
--		end,
	...
	table.insert(gal_geology.waterplus.finite_blocks,"gal:fluid_finite_"..a)
	gal_geology.register_liquid(
		":gal:fluid_finite_"..a,
		"",
...
--Create blocks
for a=1, gal_geology.waterplus.finite_water_max do
	c=c+1
	
	if c>gal_geology.waterplus.finite_water_inc_skip then
        	c = 0
        	h = h + gal_geology.waterplus.finite_water_inc
	end

	gal_geology.waterplus.register_step(a,h)
    gal_geology.waterplus.finite_water_max_id = a

end
gal_geology.waterplus.finite_water_max_name="gal:fluid_finite_"..gal_geology.waterplus.finite_water_max_id

--The ABM
minetest.register_abm({
	nodenames = gal_geology.waterplus.finite_blocks,
	interval = 1/10,
	chance = 1,
	action = function(pos,node)
		...
		dPrint("Waterplus [finite] - Calculating for "..node_id.." at "..pos.x..","..pos.y..","..pos.z)
	        ...
	        local pressure = 1
	        if minetest.env:get_node(upc).name == gal_geology.waterplus.finite_water_max_name or minetest.env:get_node(upc).name == gal_geology.waterplus.base_fluid then
		...
		if performDrop(pos,target) then 
	            if minetest.env:get_node(upc).name == gal_geology.waterplus.base_fluid then
	                minetest.env:set_node(upc,{name = gal_geology.waterplus.finite_water_max_name})
	            end
	            pos=target 
	        end
			...
			dPrint("test nei "..name ..' = '.. (target_id or 'NO'))
	            ...
	            if infected < 1 and coords[i].wi and name==gal_geology.waterplus.base_fluid and source_id<gal_geology.waterplus.finite_water_max_id then
	
			dPrint('convert up='..(coords[i].u or '')..' me=' .. source_id)
	
	                minetest.env:set_node(coords[i],{name = gal_geology.waterplus.finite_water_max_name})
	                target_id = gal_geology.waterplus.finite_water_max_id
	                --high_nearby = gal_geology.waterplus.finite_water_max_id
	                infected = infected + 1
	            end
	            if coords[i].f and name == "air" then 
	                coords[i].v = gal_geology.waterplus.finite_water_max_id 
	                coords[i].t = 0
	                can = 1
	            ...
	            elseif coords[i].f and target_id >= 1 then 
	                --coords[i].v = gal_geology.waterplus.finite_water_steps - target_id
	                ...
	                if coords[i].h then --and pressure <= 1
	                    ...
	                    coords[i].v = gal_geology.waterplus.finite_water_max_id - target_id
	                end
			...
			dPrint('test water ' .. (coords[i].wi or 'nwi') .. ' t=' .. target_id)
	                ...
	                if coords[i].h and coords[i].t >= gal_geology.waterplus.finite_water_max_id then can_max = can_max + 1 end
	            end
	            if coords[i].d and (name==gal_geology.waterplus.base_fluid) then can_min = can_min + 1 end
	
			--print('cmt me=' .. source_id .. ' to='.. (coords[i].d or 'no') .. ' n=' .. name .. ' r=' .. can_min )
	
	            if coords[i].h and coords[i].iw and ((target_id and target_id < gal_geology.waterplus.finite_water_max_id-1) or name == "air") then 
...
-- step2: perform flow and drop  twice: first for drop then flow if something rest
        	...
        	for i = 1,9 do
	            -- bubble fast up
	            if coords[i].b and coords[i].n == gal_geology.waterplus.base_fluid then
			...
			--dPrint('r1' .. "gal:fluid_finite_".."gal:fluid_finite_"..source_id)
	                ...
	                source_id = gal_geology.waterplus.finite_water_max_id
	                can_water = 1
		...
		dPrint('canmax=' .. can_max..' s='..source_id.. ' cw='..can_water)
	        ...
	        if can_max >= 1 and source_id == gal_geology.waterplus.finite_water_max_id - 1 then 
		...
		dPrint('test canwater' .. can_water ..' me='.. source_id)
	        ...
	        if can_water>0 and source_id == gal_geology.waterplus.finite_water_max_id then
	            set = gal_geology.waterplus.base_fluid
	        end
...
function performFlow(from,to)
	...
	if id > target_id and id > 0 then
		...
		if (id-1) < 1 or (target_id+1) > gal_geology.waterplus.finite_water_max then
...
function performDrop(from,to)
	...
	if target_id >= gal_geology.waterplus.finite_water_max_id then
        return
	end
    ...
    --dPrint('droptest '..target_id ..'+'.. id ..' maxid='.. gal_geology.waterplus.finite_water_max_id ..' max='.. gal_geology.waterplus.finite_water_max)
	target_id = target_id + id
	id=0

	if target_id > gal_geology.waterplus.finite_water_max_id then
	   id = target_id - gal_geology.waterplus.finite_water_max_id
	   target_id = gal_geology.waterplus.finite_water_max_id
	end
...
function pressure_get(pos, recalc)
    ...
    if not (node.name == gal_geology.waterplus.finite_water_max_name or node.name == gal_geology.waterplus.base_fluid) then return 0 end
...
minetest.register_abm({
	nodenames = {gal_geology.waterplus.base_fluid_flowing},
	interval = 1,
	chance = 1,
	action = function(pos,node)
        local level = math.floor((node.param2/15)*gal_geology.waterplus.finite_water_max_id)
        if level < 1 then level = 1 end
        if level > gal_geology.waterplus.finite_water_max_id-3 then level = gal_geology.waterplus.finite_water_max_id-3 end
