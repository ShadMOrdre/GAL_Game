...
for i, ore in ipairs(gal.lib.csv.read("|", gal_geology.path .. "/" .. gal_geology.ore_data_file .. ".csv")) do
		...
		if oretype ~= "" then
			...
			if oretype == "sheet" then
				...
				minetest.register_ore({
					...
					y_min = tonumber(ymin) - gal_geology.biome_vertical_blend,
					y_max = tonumber(ymax) + gal_geology.biome_vertical_blend,
					...
					column_height_min = tonumber(col_hght_min) or (gal_geology.biome_altitude_range / 2) + gal_geology.biome_vertical_blend,
					column_height_max = tonumber(col_hght_max) or gal_geology.biome_altitude_range + (gal_geology.biome_vertical_blend * 2),
			...
			if oretype == "stratum" then
				...
				minetest.register_ore({
					...
					wherein = "gal:" .. wherein,
					clust_scarcity = 1,
					y_min = gal_geology.maxheight_beach,
					y_max = (gal_geology.maxheight_highland - 10),
