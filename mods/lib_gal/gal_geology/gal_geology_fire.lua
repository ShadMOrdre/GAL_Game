...
gal.lib.node.register_to_game("fire_flame_basic", {
	drawtype = "firelike",
	tiles = {
		{
			name = "lib_materials_flame_basic_animated.png",
			animation = {
	...
	inventory_image = "lib_materials_flame_basic.png",
	paramtype = "light",
	light_source = 13,
	walkable = false,
	buildable_to = true,
...
gal.lib.node.register_to_game("fire_flame_large_basic", {
	drawtype = "firelike",
	tiles = {
		{
...
gal.lib.node.register_to_game("fire_flame_permanent", {
...
minetest.register_craft({
	output = "gal:tool_flint_and_steel",
	recipe = {
...
minetest.register_alias("fire:basic_flame", "gal:fire_flame_basic")
minetest.register_alias("fire:permanent_flame", "gal:fire_flame_permanent")
minetest.register_alias("fire:flint_and_steel", "gal:tool_flint_and_steel")
...
minetest.override_item("gal:mineral_coal_block", {
	after_destruct = function(pos, oldnode)
		pos.y = pos.y + 1
		if minetest.get_node(pos).name == "gal:fire_flame_permanent" then
			minetest.remove_node(pos)
		end
	end,
	on_ignite = function(pos, igniter)
		...
		if minetest.get_node(flame_pos).name == "air" then
			minetest.set_node(flame_pos, {name = "gal:fire_flame_permanent"})
		end
	end,
})
...
if flame_sound then
	...
	function fire.update_player_sound(player)
		...
		local fpos, num = minetest.find_nodes_in_area(
			areamin,
			areamax,
			{"gal:fire_flame_basic", "gal:fire_flame_permanent"}
		)
		-- Total number of flames in radius
		local flames = (num["gal:fire_flame_basic"] or 0) +
			(num["gal:fire_flame_permanent"] or 0)
		-- Stop previous sound
