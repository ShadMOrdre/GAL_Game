...
function gal_geology.fluid_short(str)
	return string.lower(str):gsub("%s", "_")
end

function gal_geology.vessels.get_is_vessel(stack)
	return minetest.get_item_group(stack:get_name(), "vessel") > 0
end

function gal_geology.vessels.get_is_empty_vessel(stack)
	return minetest.get_item_group(stack:get_name(), "vessel_blank") > 0
end

function gal_geology.vessels.get_vessel_contents(stack)
	if not gal_geology.vessels.get_is_vessel(stack) then return nil end
...
local function update_vessel(stack)
	...
	local meta = stack:get_meta()
	local contents, fluid_name, capacity = gal_geology.vessels.get_vessel_contents(stack)
...
function gal_geology.vessels.add_fluid(stack, source_name, amount)
	if not gal_geology.vessels.get_is_vessel(stack) then return nil end
	local source_node = minetest.registered_nodes[source_name]
	local fluid       = gal.lib.fluid.cleanse_node_description(source_name)
	local internal    = gal_geology.fluid_short(fluid)
...
function gal_geology.vessels.take_fluid(stack, amount)
	if not gal_geology.vessels.get_is_vessel(stack) then return nil end
...
local function register_vesselfluid(data)
	...
	local fluid       = gal.lib.fluid.cleanse_node_description(data.source_name)
	local internal    = gal_geology.fluid_short(fluid)
...
function gal_geology.vessels.register_vessel(data)
...
--		})
		gal.lib.node.register_to_game(item_name, {
			description = vessel_desc.." (Empty)\nThis item holds millibuckets of fluid.",
			drawtype = "plantlike",
	...
	if data.fluids then
		...
		for i, v in pairs(gal_geology.liquids) do
...
--gal_geology.vessels.register_vessel({
...
fluidity = {}
fluidity.fluid_short = gal_geology.fluid_short

fluidity.florbs = {}
fluidity.florbs.get_is_vessel = gal_geology.vessels.get_is_vessel
fluidity.florbs.get_is_empty_vessel = gal_geology.vessels.get_is_empty_vessel
fluidity.florbs.get_vessel_contents = gal_geology.vessels.get_vessel_contents
fluidity.florbs.add_fluid = gal_geology.vessels.add_fluid
fluidity.florbs.take_fluid = gal_geology.vessels.take_fluid
fluidity.florbs.register_vessel = gal_geology.vessels.register_vessel
