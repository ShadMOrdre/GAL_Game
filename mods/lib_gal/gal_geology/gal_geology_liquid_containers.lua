...
minetest.register_craft({
	output = 'gal:tool_bucket_empty 1',
	recipe = {
...
--Craft unfired pot
minetest.register_craft({
	output = 'gal:tool_pot_clay_unfired 1',
	recipe = {
...
minetest.register_craft({
	...
	recipe = {{'gal:tool_pot_clay_unfired'}}
})

--Cook unfired pot to give the useable bucket
minetest.register_craft({
	type = "cooking",
	output = "gal:tool_pot_clay",
	recipe = "gal:tool_pot_clay_unfired",
	cooktime = 3,
})
...
--function gal_geology.register_liquid(source, flowing, itemname, bucket_item, claypot_item, inventory_image, name, bucket_name, claypot_name, groups, force_renew)
function gal_geology.register_liquid(source, flowing, bucket_item, bucket_wood_item, bucket_steel_item, claypot_item, inv_image, bucket_name, bucket_wood_name, bucket_steel_name, claypot_name, groups, force_renew)

		gal_geology.liquids[source] = {
			source = source,
			flowing = flowing,
			bucket_item = bucket_item,
			bucket_wood_item = bucket_wood_item,
			bucket_steel_item = bucket_steel_item,
			claypot_item = claypot_item,
			force_renew = force_renew,
		}
		gal_geology.liquids[flowing] = gal_geology.liquids[source]
		...
		if bucket_item ~= nil then
			...
			minetest.register_craftitem(":"..bucket_item, {
				...
				on_place = function(itemstack, user, pointed_thing)
					...
					return ItemStack("gal:tool_bucket_empty")
				end
			})
		end
		...
		if bucket_wood_item ~= nil then
			...
			minetest.register_craftitem(":"..bucket_wood_item, {
				...
				on_place = function(itemstack, user, pointed_thing)
					...
					return ItemStack("gal:tool_bucket_wood_empty")
				end
			})
		end
		...
		if bucket_steel_item ~= nil then
			...
			minetest.register_craftitem(":"..bucket_steel_item, {
				...
				on_place = function(itemstack, user, pointed_thing)
					...
					return ItemStack("gal:tool_bucket_steel_empty")
				end
			})
		end
		...
		if claypot_item ~= nil then
			...
			minetest.register_craftitem(":"..claypot_item, {
				...
				on_place = function(itemstack, user, pointed_thing)
					...
					return ItemStack("gal:tool_pot_clay_empty")
				end
			})
		end
	--end
...
minetest.register_craftitem(":gal:tool_bucket_empty", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		local liquiddef = gal_geology.liquids[node.name]
		...
		and node.name == liquiddef.source then
			...
			if item_count > 1 then
				...
				giving_back = "gal:tool_bucket_empty "..tostring(item_count-1)
				...
				for id,fluid in pairs(buffers) do
					if fluid ~= "" then
						...
						if took.millibuckets == 1000 and took.name == fluid then
							if gal_geology.liquids[fluid] then
								itemstack = ItemStack(gal_geology.liquids[fluid].bucket_item)
								if ndef.on_timer then
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		and node.name == liquiddef.source then
			...
			if item_count > 1 then
				...
				-- set to return empty buckets minus 1
				giving_back = "gal:tool_bucket_wood_empty "..tostring(item_count-1)
			...
			if #buffers > 0 then
				for id,fluid in pairs(buffers) do
					if fluid ~= "" then
						...
						if took.millibuckets == 1000 and took.name == fluid then
							if gal_geology.liquids[fluid] then
								itemstack = ItemStack(gal_geology.liquids[fluid].bucket_wood_item)
								if ndef.on_timer then
									minetest.get_node_timer(lpos):start(ndef.node_timer_seconds or 1.0)
								end
								break
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		and node.name == liquiddef.source then
			...
			if item_count > 1 then
				...
				-- set to return empty buckets minus 1
				giving_back = "gal:tool_bucket_steel_empty "..tostring(item_count-1)
			...
			if #buffers > 0 then
				for id,fluid in pairs(buffers) do
					if fluid ~= "" then
						...
						if took.millibuckets == 1000 and took.name == fluid then
							if gal_geology.liquids[fluid] then
								itemstack = ItemStack(gal_geology.liquids[fluid].bucket_steel_item)
								if ndef.on_timer then
									minetest.get_node_timer(lpos):start(ndef.node_timer_seconds or 1.0)
								end
								break
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		and node.name == liquiddef.source then
			...
			if item_count > 1 then
				...
				-- set to return empty buckets minus 1
				giving_back = "gal:tool_pot_clay_empty "..tostring(item_count-1)
			...
			if #buffers > 0 then
				for id,fluid in pairs(buffers) do
					if fluid ~= "" then
						...
						if took.millibuckets == 1000 and took.name == fluid then
							if gal_geology.liquids[fluid] then
								itemstack = ItemStack(gal_geology.liquids[fluid].claypot_item)
								if ndef.on_timer then
									minetest.get_node_timer(lpos):start(ndef.node_timer_seconds or 1.0)
								end
								break
...
for i, liq in ipairs(gal.lib.csv.read("|", gal_geology.path .. "/gal_geology_containers.csv")) do
	...
	if string.find(liqsrc, "water") then
		if string.find(liqsrc, "river") then
			gal_geology.register_liquid(
				"gal:" .. liqsrc,
				"gal:" .. liqflo,
				"gal:tool_bucket_" .. liqbuc,
				"gal:tool_bucket_wood_" .. liqbuc,
				"gal:tool_bucket_steel_" .. liqbuc,
				"gal:tool_pot_clay_" .. liqpot,
				liqimg,
	...
	--minetest.log(S("[MOD] gal:  dump gal_geology.liquids: " .. dump(gal_geology.liquids)))

--Register lava buckets as a fuel source
minetest.register_craft({
	type = "fuel",
	recipe = "gal:tool_bucket_lava",
	burntime = 60,
	replacements = {{"gal:tool_bucket_lava", "gal:tool_bucket_empty"}},
})
minetest.register_craft({
	type = "fuel",
	recipe = "gal:tool_pot_clay_lava",
	burntime = 60,
	replacements = {{"gal:tool_pot_clay_lava", "gal:tool_pot_clay_empty"}},
})
minetest.register_craft({
	type = "fuel",
	recipe = "gal:tool_bucket_steel_lava",
	burntime = 60,
	replacements = {{"gal:tool_bucket_steel_lava", "gal:tool_bucket_steel_empty"}},
})
...
bucket = {}
bucket.liquids = gal_geology.liquids

minetest.register_alias("bucket",       "gal:tool_bucket_empty")
minetest.register_alias("bucket_water", "gal:tool_bucket_water")
minetest.register_alias("bucket_lava",  "gal:tool_bucket_lava")
