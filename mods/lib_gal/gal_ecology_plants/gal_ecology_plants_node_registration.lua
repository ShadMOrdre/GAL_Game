...
local S = gal_ecology_plants.intllib
...
for i, stone in ipairs(gal.lib.csv.read("|", gal_ecology_plants.path .. "/" .. gal_ecology_plants.nodes_data_file .. ".csv")) do
...
--USE MAX LIGHT FOR CROPS
	...
	if string.find(node_name, "_sapling") then
		...
		new_node_def.on_timer = function(pos)
			if not gal_ecology_plants.can_grow(pos) then
			...
			if string.find(grow, ",") then
				...
				minetest.place_schematic(pos, gal_ecology_plants.schematics.select(rname), "random", nil, true, "place_center_x, place_center_z")
			end
		end
		new_node_def.on_place = function(itemstack, placer, pointed_thing)
			itemstack = gal_ecology_plants.sapling_on_place(itemstack, placer, pointed_thing,
				"gal:" .. node_name .. "",
