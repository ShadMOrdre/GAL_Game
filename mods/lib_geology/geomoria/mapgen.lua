...
local ground_nodes_names = {
	"gal:stone_desert",			--'default:desert_stone',
	"gal:dirt",				--'default:dirt',
	"gal:dirt_with_grass_dry",		--'default:dirt_with_dry_grass',
	"gal:dirt_with_grass",			--'default:dirt_with_grass',
	"gal:dirt_with_snow",			--'default:dirt_with_snow',
	"gal:sand",				--'default:sand',
	"gal:stone_sandstone",			--'default:sandstone',
	"gal:stone",				--'default:stone',
}
...
local function generate(p_minp, p_maxp, seed)
	...
	if exit_stair and minp.y < 200 and avg > geomoria_depth * 80 - 32 then
		heightmap = minetest.get_mapgen_object("heightmap") or gal.mapgen.heightmap
		...
		for z = minp.z, maxp.z do
			for y = minp.y, maxp.y do
				...
				for x = minp.x, maxp.x do
					...
					--local biome_name = biome_map[ni2d]
					local biome_name = biome_map[index2d]
					local biome_stone
					local biome_dungeon
					local biome_dungeon_alt
					if biome_name ~= "" then
						biome_stone = biome_data[biome_name].b_stone or node[N["S0"]]
						biome_dungeon = biome_data[biome_name].b_dungeon or node[N["CC"]]
						biome_dungeon_alt = biome_data[biome_name].b_dungeon_alt or node[N["CM"]]
					end
					...
					if damage > geomoria_mod.damage_level then
						if data[ivm] ~= node[N["WS"]] and data[ivm] ~= node[N["WS"]] then
							--ShadMOrdre 20180821
							--data[ivm] = node['air']
						end
					elseif (data[ivm] == node[N["S0"]] or data[ivm] == node[N["SB"]] or data[ivm] == biome_stone) and damage > geomoria_mod.damage_level - 0.5 then
						if (wetness > 0 and math_random(2) == 1) or (wetness == 0 and math_random(10) == 1) then
							...
							--if minetest.registered_items['lib_materials:mineral_salt_block'] and math_random(4) == 1 then
							if math_random(100) == 1 then
								--ShadMOrdre 20180821
								--data[ivm] = node['fun_caves:glowing_fungal_stone']
--[[
								local cryst_rnd = math_random(12)
								if cryst_rnd == 2 then
									data[ivm] = node['lib_materials:mineral_amethyst_crystal_block']
								elseif cryst_rnd == 4 then
									data[ivm] = node['lib_materials:mineral_emerald_crystal_block']
								elseif cryst_rnd == 6 then
									data[ivm] = node['lib_materials:mineral_quartz_block']
								elseif cryst_rnd == 8 then
									data[ivm] = node['lib_materials:mineral_ruby_crystal_block']
								elseif cryst_rnd == 10 then
									data[ivm] = node['lib_materials:mineral_saphire_crystal_block']
								else
									data[ivm] = node['lib_materials:mineral_salt_block']
								end
--]]
							else
								--ShadMOrdre 20191019
								--local biome_id = minetest.get_biome_data(minp.x, y, z)
								--local biome_dungeon_stone = minetest.registered_biomes[minetest.get_biome_name(biome_id)].node_dungeon or minetest.registered_biomes[minetest.get_biome_name(biome_id)].node_dungeon_alt or minetest.registered_biomes[minetest.get_biome_name(biome_id)].node_stone
								--if biome_dungeon_stone then
								--	data[ivm] = node[biome_dungeon_stone]
								--else
									--data[ivm] = node[N["SM"]]
									data[ivm] = biome_dungeon_alt
								--end
							end
						else
							--data[ivm] = node[N["SK"]]
							data[ivm] = biome_dungeon
						end
					end
