...
minetest.log(S("[MOD] lib_mg_v3d:  Version:") .. S(mg_v3d.ver_str))
minetest.log(S("[MOD] lib_mg_v3d:  Legal Info: Copyright ") .. S(mg_v3d.copyright) .. " " .. S(mg_v3d.authorship) .. "")
minetest.log(S("[MOD] lib_mg_v3d:  License: ") .. S(mg_v3d.license) .. "")
...
--[[
	local function calc_biome_from_noise(heat, humid, pos)
		...
		for i, biome in pairs(mg_v3d.biome_info) do
...
--]]
	...
	minetest.register_on_generated(function(minp, maxp, seed)
		...
		nobj_2dterrain = nobj_2dterrain or minetest.get_perlin_map(np_2dterrain, permapdims2d)
		--nbuf_2dterrain = nobj_2dterrain:get_2d_map(minpos2d)
		local nvals_2dterrain = nobj_2dterrain:get_2d_map(minpos2d, nbuf_2dterrain)

		nobj_v3d_fill = nobj_v3d_fill or minetest.get_perlin_map(np_v3d_fill, permapdims3d)
		local nvals_v3d_fill = nobj_v3d_fill:get_3d_map(minpos3d, nbuf_v3d_fill)

		nobj_filler_depth = nobj_filler_depth or minetest.get_perlin_map(np_v3d_filler_depth, chulens)
...
--
				...
				local n_s = nvals_filler_depth[z-minp.z+1][x-minp.x+1]

				local n_f = minetest.get_perlin(np_3dterrain):get_3d({x = x, y = n_y, z = z})

				local s_d = n_f - n_y
				local n_t = n_s * n_f - s_d

--[[
				--local density_noise = minetest.get_perlin(np_v3d_fill):get_3d({x = x, y = n_y, z = z})

				--local density_gradient = (1 - n_y) / (mg_v3d.density * mg_v3d.mg_world_scale)
				--local density_height = density_noise + density_gradient

				--local t_y = density_height
--]]
...
--
				...
				--	gal.mapgen.heightmap[index2d] = t_y
				--else
				--	--gal.mapgen.heightmap[index2d] = -31000
				--end
...
--[
					...
					local density_noise = nvals_3dterrain[z-minp.z+1][y-minp.y+1][x-minp.x+1]

					local density_gradient = (1 - y) / (mg_v3d.density * mg_v3d.mg_world_scale)
					local density_height = density_noise + density_gradient
...
--]]
					...
					local s_d = n_f - theight
					local n_t = t_fillmap * n_f - s_d
...
--[[
					if density_height > 0 then
						write_3d = true
					elseif y <= mg_v3d.water_level then
						if theight <= mg_v3d.water_level then
							write_3d = true
						end
						if theight <= (minp.y + 1) then
							write_3d = true
						end
					end
--]]
