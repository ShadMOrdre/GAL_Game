...
if gal.mapgen.name ~= "mg_valleys2d" then
	return
end

mg_valleys2d = {}
mg_valleys2d.name = "mg_valleys2d"
mg_valleys2d.ver_max = 0
mg_valleys2d.ver_min = 1
mg_valleys2d.ver_rev = 0
mg_valleys2d.ver_str = mg_valleys2d.ver_max .. "." .. mg_valleys2d.ver_min .. "." .. mg_valleys2d.ver_rev
mg_valleys2d.authorship = "ShadMOrdre.  Additional credits to Termos' Islands mod; Gael-de-Sailleys' Valleys; duane-r Valleys_c, burli mapgen, and paramats' mapgens"
mg_valleys2d.license = "LGLv2.1"
mg_valleys2d.copyright = "2020"
mg_valleys2d.path_mod = minetest.get_modpath(minetest.get_current_modname())
mg_valleys2d.path_world = minetest.get_worldpath()

mg_valleys2d.intllib = gal.intllib
local S = gal.intllib

minetest.log(S("[MOD] mg_valleys2d:  Loading..."))
minetest.log(S("[MOD] mg_valleys2d:  Version:") .. S(mg_valleys2d.ver_str))
minetest.log(S("[MOD] mg_valleys2d:  Legal Info: Copyright ") .. S(mg_valleys2d.copyright) .. " " .. S(mg_valleys2d.authorship) .. "")
minetest.log(S("[MOD] mg_valleys2d:  License: ") .. S(mg_valleys2d.license) .. "")
	...
	mg_valleys2d.fillermap = {}
	mg_valleys2d.rivermap = {}
	mg_valleys2d.surfacemap = {}
	mg_valleys2d.slopemap = {}

	mg_valleys2d.water_level = 1
	mg_valleys2d.use_heat_scalar = false

	mg_valleys2d.mg_world_scale = 1
	local mg_world_scale = mg_valleys2d.mg_world_scale
	mg_valleys2d.mg_river_size = 5
	mg_valleys2d.mg_dim = "2d"
	local max_highland = gal.mapgen.maxheight_highland
	local max_mountain = gal.mapgen.maxheight_mountain


	--mg_valleys2d.mg_noise_spread = 600
	--mg_valleys2d.mg_noise_scale = 25
	--mg_valleys2d.mg_noise_lacunarity = 2.19
	--mg_valleys2d.mg_noise_seed = 5934
	----mg_valleys2d.mg_noise_offset = 0
	--mg_valleys2d.mg_noise_offset = -4
	--mg_valleys2d.mg_noise_octaves = 8
	--mg_valleys2d.mg_noise_persist = 0.3
	--mg_valleys2d.mg_noise_lacunarity = 2.19

	--mg_valleys2d.mg_noise_spread = 512
	--mg_valleys2d.mg_noise_scale = 25
	--mg_valleys2d.mg_noise_seed = 5202
	----mg_valleys2d.mg_noise_offset = 0
	--mg_valleys2d.mg_noise_offset = -5
...
--]]
	...
	local function get_heat_scalar(z)
		...
		if mg_valleys2d.use_heat_scalar == true then
			local t_z = abs(z)
			local t_heat = 0
			local t_heat_scale = 0.0071875 
			local t_heat_factor = 0
	
			--local t_heat_mid = ((mg_valleys2d.mg_map_size * mg_valleys2d.mg_world_scale) * 0.25)
...
--[[
	local function get_valleys_height(z,x,dim)
		...
		-- Mapgen parameters
		local river_size_factor = mg_valleys2d.mgvalleys_river_size / 100
		local water_level       = mg_valleys2d.water_level
...
--]]
	...
	minetest.register_on_generated(function(minp, maxp, seed)
		...
		nbuf_val_fill = nobj_val_fill:get_2d_map(minpos2d)
		...
		-- Mapgen parameters
		local river_size_factor = mg_valleys2d.mg_river_size / 100
		local water_level       = mg_valleys2d.water_level
		local mg_dim = mg_valleys2d.mg_dim	
	...
	--## HEIGHTMAP CALCULATION
				...
				--local n_fill = nbuf_val_fill[z-minp.z+1][x-minp.x+1]
	...
	--## RIVERS CALCULATION
				--river_size_factor = (mg_valleys2d.mg_river_size - (y * 0.025)) / 100
				local river_course
				if abs_rivers <= river_size_factor then
					-- TODO: Add riverbed calculation
					river_course = abs_rivers
				end
				mg_valleys2d.rivermap[index2d] = river_course

	--## FILLER CALCULATION

				mg_valleys2d.fillermap[index2d] = nbuf_filler_depth[z-minp.z+1][x-minp.x+1]
				--mg_valleys2d.fillermap[index2d] = nbuf_filler_depth[z-minp.z+1][x-minp.x+1] + nbuf_val_dirt[z-minp.z+1][x-minp.x+1]
...
--## SPAWN SELECTION
		...
		for z = minp.z, maxp.z do
			for y = minp.y, maxp.y do
				for x = minp.x, maxp.x do
					...
					local t_fillmap = mg_valleys2d.fillermap[index2d]
					local t_biome_name = gal.mapgen.biomemap[index2d]
					local t_rivermap = mg_valleys2d.rivermap[index2d] or (river_size_factor + 1)
	...
	--## BIOME GENERATION
					...
					river_size_factor = (mg_valleys2d.mg_river_size - (theight / (40 * mg_world_scale))) / 100
	...
	--## NODE PLACEMENT FROM HEIGHTMAP
				...
				--2D Terrain
					...
					elseif y >= (theight - (fill_depth + top_depth)) and y < (theight - top_depth) then
						if t_rivermap <= river_size_factor then
							if y > (mg_valleys2d.water_level - 1) then
					...
					elseif y >= (theight - top_depth) and y <= theight then
						if t_rivermap <= river_size_factor then
							if y > mg_valleys2d.water_level then
								t_top = t_air
							else
								t_top = t_water
							end
						...
						t_node = t_top
					elseif y > theight and y <= mg_valleys2d.water_level then
		...
		print ("[mg_valleys2d] Mapchunk generation time " .. chugent .. " ms")
		...
		if mem > 1000 then
			print("mg_valleys2d is manually collecting garbage as memory use has exceeded 500K.")
	...
	minetest.register_on_shutdown(function()
		...
		minetest.log("mg_valleys2d lua Mapgen Times:")
...
minetest.log(S("[MOD] mg_valleys2d:  Successfully loaded."))
