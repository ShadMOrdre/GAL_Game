...
elseif rawget(_G, "gal.sfinv") then --sfinv installed (part of minetest_game since 0.4.15)
--]]
	assert(gal.sfinv.enabled)
	local orig_get = gal.sfinv.pages["sfinv:crafting"].get
	gal.sfinv.override_page("sfinv:crafting", {
	...
	local set_page = gal.sfinv.set_page or function(player, name)
		--assumptions: src pg has no leave callback, dst pg has no enter callback
		local ctx = {page=name}
		gal.sfinv.contexts[player:get_player_name()] = ctx
		gal.sfinv.set_player_inventory_formspec(player, ctx)
	end
